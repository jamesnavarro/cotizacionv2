<?php
	...
	if ($_GET['tipo'] == 'vidrio') {
		...
		$query_select_vidrio = mysqli_query($conexion,"SELECT * FROM producto WHERE id_p = " . $id_vidrio_servicio);
		$row_vidrio = mysqli_fetch_array($query_select_vidrio);
		$query_select_espesor = mysqli_query($conexion,"SELECT * FROM tipo_vidrio WHERE id_vidrio = " . $id_espesor_servicio);
		$row_espesor = mysqli_fetch_array($query_select_espesor);
		mysqli_query($conexion,"INSERT INTO info_servicios (id_cot, id_servicio, linea, id_prod, perforacion, boquete, cantidad_prod, precio_unitario, porcentaje, medida1, medida2, medida3, color_prod, guardado, fecha_registro, usuario)
		...
		$query_select_color = mysqli_query($conexion,"SELECT * FROM tipo_aluminio WHERE id_ta = " . $color_aluminio);
		$row_color = mysqli_fetch_array($query_select_color);
		$query_select_porcentaje = mysqli_query($conexion,"SELECT * FROM porcentajes WHERE area_por = 'MP' AND grupo = '" . $grupo_hidden . "'");
		$row_porcentaje = mysqli_fetch_array($query_select_porcentaje);
		...
		mysqli_query($conexion,"INSERT INTO info_servicios (id_cot, id_servicio, linea, id_prod, perforacion, boquete, cantidad_prod, precio_unitario, precio_porcentaje, porcentaje, medida1, medida2, medida3, color_prod, guardado, fecha_registro, usuario)
										 VALUES ('" . $id_cot . "', '0', 'Accesorio', '" . $id_accesorio_servicio . "', '0', '0', '" . $cant_accesorio . "', '" . $valor_accesorio_hidden . "', '" . $valor_venta . "', '" . $porcentaje_servicio_accesorio .
										 		  ...
										 		  "', '0', '" . date('Y-m-d H:i:s') . "', '" . $_SESSION['k_username'] . "')");
		$query_select_info_accesorios = mysqli_query($conexion,"SELECT * FROM info_servicios WHERE id_cot = " . $id_cot . " AND linea = 'Accesorio'");

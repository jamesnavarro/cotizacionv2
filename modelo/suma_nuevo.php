...
$alum_por = "SELECT (".$precio_mp.") as p FROM porcentajes where area_por='MP' and grupo='Perfileria'";
                ...
                $alum_porr = "SELECT costo_a FROM tipo_aluminio where color_a='".$alum."'";
...
//            echo '<script>alert("B cant1: '.number_format($cantid).'")</script>'; 
              ...
              if($perimetro=='0'){
                  ...
                  $valor_acabado = $vc * $perimetro * $cantidad;
              }
            ...
            echo '<script> console.log('.$precio_actual.' x '.$cantid.' x '.($al/1000).' + '.$valor_acabado.' = '.$costo_und.');</script>';
            // costos fom 
            $mpfom = $precio_actual_fom/$porcaB; 
            $tafom = $tafom + ($al*$mpfom*(($d))/1000) + $valor_acabado;
...
//                echo ($taa*($row["costo_mt"]/$porcacc)*$m*$_POST['cant'] + $a).'<br>';
  ...
  while($row = mysqli_fetch_array($request_acE2)){
      ...
      $mt2 = (($_POST['alto']/1000)*($_POST['ancho']/1000)) * $lam;

...
//            echo '<script>alert("B cant1: '.number_format($cantid).'")</script>'; 
              ...
              if($perimetro=='0'){
                  ...
                  $valor_acabado = $vc * $perimetro * ($al/1000) * $cantid;
              }
              $total_acabado += $valor_acabado;
              $mp = $precio_actual/$porca;
              
              //formulas nuevas ------------16/09/2017
           $costo_und = ($precio_actual * $cantid * ($al/1000)) + $valor_acabado;
           $costo_tot = $costo_und * $_POST['cant'];
           //fin costo
           $costo_und_desp = $costo_und / $porca;
           $costo_tot_desp = $costo_und_desp * $_POST['cant'];
           ...
           $vr4 += $costo_tot_desp;
            ...
            $ta += ($al*$mp*(($d))/$n);
             echo '<script> console.log('.$costo_tot.');</script>';
            // costos fom 
...
if($total_acabado==0){
 ...
 echo '<script> console.log('.$vr2.');</script>';

$a2 = $tafom  + $total_acabado;
$a3 = ($ta*$porca)  + $total_acabado;
$a4 = ($tafom * $porcaB)  + $total_acabado;
$perfiles = $vr4;
$perfiles_sinp = $vr2;
$perfiles_fom = $tafom + $a2;
$perfiles_fom_sinp = ($tafom * $porcaB) + $a4;
////    PERFILES     ----------------------------------------------<-<-<<<<-<<--<>>>>
 echo '<script> console.log('.$perfiles.');</script>';
...
if($request_rej){
  ...
  {      
       ...
       $request_vrej=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_vid b, referencias c where b.id_ref_vid=c.id_referencia and a.id_p=b.id_p and a.id_p=".$_POST['ref']." and b.id_r_v=".$row["medida_rej"]." ");
         ...
         $prejfom = $row["costo_fom"] / $porcaB;  
          $precio_actual = $row["costo_mt"];
           ...
           //fin de cobro
            ...
            $valor_t = $valor_u * $_POST['cant'];
            
            $valor_und_desp = $valor_u / $porca;
            $valor_tot_desp = $valor_und_desp * $_POST['cant'];
            
            $xx = $xx + $valor_tot_desp ;
;
            //costos fom
...
// FIN REJILLASSSS----------------------------------------<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>
 $acc_por = "SELECT (".$precio_mp.") as p FROM porcentajes where area_por='MP' and grupo='Accesorios'";
  ...
  $request_acE=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_acc b, referencias c where b.id_ref_acc=c.id_referencia and a.id_p=b.id_p and a.id_p=".$_POST["ref"]." order by b.para ");
  ...
  {  
       //--------------------------------------------------------------------
    if($row['can_rej']!=0){
    $request_v2=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_rej b, referencias c where b.id_referencia=c.id_referencia and a.id_p=b.id_p and a.id_p=".$_POST['ref']." and b.id_r_rej=".$row['can_rej']." ");
...
//            }else{      
            ...
            if($_POST['pelicula']!="No Aplica"  && $row['codigo']=='26044'){
...
//                echo ($taa*($row["costo_mt"]/$porcacc)*$m*$_POST['cant'] + $a).'<br>';
          ...
          $request_acE2=mysqli_query($conexion,"SELECT * FROM referencias where id_referencia='".$pel."' ");
  ...
  while($row = mysqli_fetch_array($request_acE2)){
      ...
      $mt2 = (($_POST['alto']/1000)*($_POST['ancho']/1000)) * $cantlam;
      $cos_und = $row["costo_mt"] * $mt2;
      $cos_tot = $cos_und * $_POST['cant'];
      $vlr_und = $cos_und / $porcacc;
      $vlr_tot = $vlr_und * $_POST['cant'];
      $tac += $vlr_tot;
      
      $cosf_und = $row["costo_fom"] * $mt2;
      $cosf_tot = $cosf_und * $_POST['cant'];
      $vlrf_und = $cosf_und / $porcaccB;
      $vlrf_tot = $vlrf_und * $_POST['cant'];
      $tacfom += $vlrf_tot;
      //echo '<script>alert("pel: '.$pel.'- lamina:'.$laminas.' - valor: '.$cos_tot.' tpo : '.$vlr_tot.' porc: '.$porcacc.'")</script>';
...
// fin mano de MAQUINARIA-------------------------------<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>
           ...
           $req=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_mo b, referencia_mo c where b.id_ref_mo=c.id_ref_mo and a.id_p=b.id_p and a.id_p=".$_POST["ref"]);
if($req){

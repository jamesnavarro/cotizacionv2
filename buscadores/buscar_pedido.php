...
if($_POST['cot']==''  && $_POST['nom']=='' && $_POST['obr']==''&& $_POST['reg']=='' ){
    $request=mysql_query("SELECT *, (select concat(nombre,' ',apellido) from usuarios where id_user=aprobado_por) as nomb, (select sum(cant_restante) from cotizaciones where id_cot=a.id_cot) as cr FROM cotizacion a, cont_terceros b where a.estado='Aprobado' and  a.id_tercero=b.id_ter order by a.numero_cotizacion desc LIMIT 30 ");
}else{
$request=mysql_query("SELECT *, (select concat(nombre,' ',apellido) from usuarios where id_user=aprobado_por) as nomb, (select sum(cant_restante) from cotizaciones where id_cot=a.id_cot) as cr FROM cotizacion a, cont_terceros b where a.estado='Aprobado' and a.id_tercero=b.id_ter $cot $obr $reg");
}

...
<?php
if($_POST['cot']!=''){ $cot = ' cod_ter like "%'.$_POST['cot'].'%"'; }else{$cot = ''; }
if($_POST['nom']!=''){ $nom = ' nom_ter like "%'.$_POST['nom'].'%"'; }else{$nom = ''; }
if($_POST['cot']=='' && $_POST['nom']=='' ){
    $request2=mysql_query("SELECT * FROM cont_terceros order by nom_ter asc LIMIT 30 ");
}else{
    if($_POST['cot']==''&& $_POST['nom']!='' ){
   
    $request2=mysql_query("SELECT * FROM cont_terceros where $nom "); 
}
    if($_POST['nom']==''&& $_POST['cot']!='' ){
   
    $request2=mysql_query("SELECT * FROM cont_terceros where $cot "); 
    }
if($_POST['cot']!='' && $_POST['nom']!='' ){
   $request2=mysql_query("SELECT * FROM cont_terceros where $cot and $nom "); 
}

}
	while($rowk=mysql_fetch_array($request2))
	{       
           ?> 
           <tr>

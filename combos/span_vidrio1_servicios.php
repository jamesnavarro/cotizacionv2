...
function pasar(){
    window.opener.infoTipoEspesor(document.getElementById('datos1').value,
...
$request=mysql_query("SELECT * FROM `tipo_vidrio`");
if($request){
//    echo'<hr>';
...
//        require '../modelo/consulta_empresa.php';
	while($row=mysql_fetch_array($request))
	{       
      $table = $table.'<tr><td><a href="../combos/span_vidrio1_servicios.php?codigo='.$row["id_vidrio"].'">'.$row["color_v"].'</a></td>
                   <td>'.$row['espesor_v'].'</td>

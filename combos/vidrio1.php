...
function pasarvidrio(){
    window.opener.refvidrios1(document.getElementById('datos1').value,
    document.getElementById('datos2').value,document.getElementById('datos3').value);
    window.close();
}
</script>
</head>
<body onload="javascript:pasarvidrio();">
<?php 
...
<input type="text" name="datos3" id="datos3" readonly value="0" />
                     ...
                     $request=mysqli_query($conexion,'SELECT * FROM `producto` WHERE id_p="'.$_GET['codigo'].'"');
...
<a href="" title="pasar valor" onload="javascript:pasarvidrio();"><input type="button" name="cerrar" value="Cargar"  onclick="window.close();"></a>  
...
$request=mysqli_query($conexion,"SELECT * FROM `producto` where linea='Vidrio' order by id_p asc");
if($request){
//    echo'<hr>';
...
$table = $table.'<tr><td>0</td>'
              . '<td>0000</td>'
              . '<td><a href="../combos/vidrio1.php?codigo=0">No Tiene vidrio</a></td></tr>';
$c = 0;	
while($row=mysqli_fetch_array($request))
	{ 
    $c += 1;
      $table = $table.'<tr><td>'.$c.'</td>'
              . '<td>'.$row["referencia_p"].'</td>'
              . '<td><a href="../combos/vidrio1.php?codigo='.$row["id_p"].'">'.$row["producto"].'</a></td>

			...
			<section class="body">
				<div class="body-inner">
					<div class="span12 widget dark stacked">
						<header>
							<?php
								$query_maquina = mysql_query("SELECT * FROM subproceso_maq WHERE id_spm = '" . $_GET['id_maquina'] . "'");
								$row_maquina = mysql_fetch_array($query_maquina);
							?>
																...
																<table class="table table-bordered table-striped table-hover">
																	...
																	<tbody>
																		<?php
																			$i = 0;
																			$sw = 0;
																			$query_grupos = mysql_query("SELECT * FROM grupo WHERE id_area = '" . $_GET['id_subpro'] . "'");
																			while ($row_grupos = mysql_fetch_assoc($query_grupos)) {
																				$i = $i + 1;
																				echo "<tr>";
																				echo "<td width='5%'>" . $i . "</td>";
																				$query_verificar_grupo = mysql_query("SELECT * FROM subproceso_maq WHERE id_g = '" . $row_grupos['id_g'] . "'");
																				$row_verificar_grupo = mysql_fetch_array($query_verificar_grupo);
																				$query_id_g = mysql_query("SELECT * FROM subproceso_maq WHERE id_spm = '" . $_GET['id_maquina'] . "' AND id_g = '" . $row_grupos['id_g'] . "'");
																				$row_id_g = mysql_fetch_array($query_id_g);
																				if (mysql_num_rows($query_verificar_grupo) != 0) {

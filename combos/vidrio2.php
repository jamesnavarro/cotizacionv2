...
function pasarvidrio(){
    window.opener.refvidrios2(document.getElementById('datos1').value,document.getElementById('datos2').value,
...
$request=mysqli_query($conexion,"SELECT * FROM `producto` where linea='Vidrio'");
if($request){
//    echo'<hr>';
...
$table = $table.'<tr><td>0</td>'
              . '<td>0000</td>'
              . '<td><a href="../combos/vidrio2.php?codigo=0">No Tiene vidrio</a></td></tr>';	
        
	//Por cada resultado pintamos una linea
//        require '../modelo/consulta_empresa.php';
	while($row=mysqli_fetch_array($request))
	{       
      $table = $table.'<tr><td>'.$row["id_p"].'</td>'
              . '<td>'.$row["referencia_p"].'</td>'
              . '<td><a href="../combos/vidrio2.php?codigo='.$row["id_p"].'">'.$row["producto"].'</a></td>

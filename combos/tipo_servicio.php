				...
				<section class="body">
					<div class="body-inner no-padding">
						...
						<?php
								...
								$request = mysql_query('SELECT * FROM `referencia_mo` WHERE id_ref_mo = "' . $_GET['codigo'] . '"');
								while ($row = mysql_fetch_array($request)) {
									$x = $row["id_ref_mo"];
									$y = $row["descripcion_mo"];
									$w = $row["valor_mo"];
									$request2 = mysql_query("SELECT * FROM gastos_serv a, referencia_admin c WHERE a.id_administrativo = c.id_ref_ad AND a.id_ref = " . $row["id_ref_mo"]);
						...
						<?php
							...
							$request = mysql_query("SELECT * FROM `referencia_mo` WHERE instalacion = 'Si' ORDER BY id_ref_mo ASC");
							if ($request) {
								//echo'<hr>';

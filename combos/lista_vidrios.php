...
function pasar(){
    window.opener.obtenervidrios(document.getElementById('datos1').value,
    document.getElementById('datos2').value,document.getElementById('item').value);
    window.close();
}
</script>
</head>
...
?>
     ...
     <?php if(isset($_GET['codigo'])){
                     ...
                     $request=mysql_query('SELECT * FROM `tipo_vidrio` WHERE id_vidrio="'.$_GET['codigo'].'"');
...
<input type="text" name="item" id="item" readonly value="<?php echo $_GET['item'] ?>" />
...
$request=mysql_query("SELECT * FROM `tipo_vidrio` where estado=0");
if($request){
//    echo'<hr>';
...
//        require '../modelo/consulta_empresa.php';
	while($row=mysql_fetch_array($request))
	{       
      $table = $table.'<tr><td><a href="../combos/lista_vidrios.php?codigo='.$row["id_vidrio"].'&item='.$_GET["item"].'">'.$row["color_v"].'</a></td>
                   <td>'.$row['espesor_v'].'</td>

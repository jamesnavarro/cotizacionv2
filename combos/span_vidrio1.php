...
function pasar(){
    window.opener.vidrios1(document.getElementById('datos1').value,
...
$request=mysql_query("SELECT * FROM `tipo_vidrio` where estado=0 order by id_vidrio asc");
if($request){
//    echo'<hr>';
...
//        require '../modelo/consulta_empresa.php';
	while($row=mysql_fetch_array($request))
	{       
      $table = $table.'<tr><td><a href="../combos/span_vidrio1.php?codigo='.$row["id_vidrio"].'">'.$row["color_v"].'</a></td>
                   <td>'.$row['espesor_v'].'</td>

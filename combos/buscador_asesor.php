<?php
         ...
         if(isset($_POST['asesor'])){
              $request=mysqli_query($conexion,'SELECT * FROM `usuarios` where concat(nombre," ",apellido) like "%'.$_POST['asesor'].'%" ');
         }else{
              $request=mysqli_query($conexion,'SELECT * FROM `usuarios` where estado="Activo" and area != "Produccion" order by nombre asc ');
         }
    ?>

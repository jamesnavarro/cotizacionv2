...
//                       } 
	...
	$request = mysql_query("SELECT count(*), sum(vtotal_obra) FROM  seguimientos_cot where borrador='$sin_lista' $tobra $est $cot $valor $nom $obr $reg $ciu $fecha "   );
	if ($request) {
...
<?php
	...
	$request_ac = mysql_query("SELECT * FROM  seguimientos_cot  where borrador='$sin_lista' $tobra $est $cot $nom $valor $obr $reg $ciu $fecha order by $nombre_ord $asc_desc " . $limit);

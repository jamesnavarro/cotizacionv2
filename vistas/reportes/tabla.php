...
//                       } 
	...
	$request = mysqli_query($conexion,'SELECT id_cot, sum(costo_total) FROM cotizacion a, cont_terceros b WHERE  a.id_cot IN (SELECT max(id_cot) FROM `cotizacion` GROUP by numero_cotizacion) and a.id_tercero = b.id_ter ' . $cot . $nom . $obr . $reg  . $est . $valor. $ciu.$fecha. $qseg.' group by numero_cotizacion '   );
	if ($request) {
		$req = mysqli_num_rows($request);
                $tt = 0;
                while($sum = mysqli_fetch_row($request)){
                    $tt += $sum[1];
                }
		$num_items = $req;
	} else {
...
<table class="<?php echo $class; ?>">
            ...
            <?php
               $footer = '<tfoot><tr><td colspan="10"><a href="../reportes/excel.php?cot='.$_POST['cot'].'&fuente='.$_POST['fuente'].'&pref='.$_POST['pref'].'&bus_tip='.$_POST['bus_tip'].'&sin_lista='.$_POST['sin_lista'].'&vcot='.$_POST['vcot'].'&seg='.$_POST['seg'].'&lista='.$_POST['lista'].'&vciu='.$_POST['vciu'].'&vdir='.$_POST['vdir'].'&vest='.$_POST['vest'].'&vase='.$_POST['vase'].'&vfec='.$_POST['vfec'].'&vopc='.$_POST['vopc'].'&ano='.$_POST['ano'].'&mes='.$_POST['mes'].'&cli='.$_POST['cli'].'&obr='.$_POST['obr'].'&est='.$_POST['est'].'&pre='.$_POST['pre'].'&ase='.$_POST['ase'].'&ciu='.$_POST['ciu'].'&ord='.$_POST['ord'].'&orden='.$_POST['orden'].'&ver_tip='.$_POST['ver_tip'].'">Descargar Excel</a><div class="links">';
        if ($page > 1) {

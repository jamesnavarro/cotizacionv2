...
?>
  ...
  <tr class="table-hover">
    ...
    <?php
    $cons = mysql_query("select numero_cotizacion from cotizacion where id_cot='".$_GET['rad']."' ");
    $c = mysql_fetch_array($cons);
    $result = mysql_query("select id_cot, version, costo_total from cotizacion where numero_cotizacion='".$c[0]."' order by version desc ");//45897
    while($r = mysql_fetch_row($result)){
        $cot = $r[0];
        echo '<tr><td><center><b>Cot No. '.$c[0].'.'.$r[1].'</b> <img src="../../imagenes/imp.png"  onclick="cot('.$cot.');" style="cursor:pointer"> </center> </td>';
        $query = mysql_query("SELECT * FROM  seguimientos  where id_s = ".$cot." order by id_seguimiento desc ");
        ...
        if($cont==0){
            echo '<tr><td><font color="red"><b>Esta version no tiene seguimiento </b><img src="../../imagenes/cambiar.png" onclick="seguir('.$cot.');" style="cursor:pointer"></font>'
                    . '';
             $consulta = mysql_query("select borrador, estado_cot_s, nombre_obra, count(id_s) from seguimientos_cot where id_relacion='$cot' ");
            $se = mysql_fetch_row($consulta);
            if($se[3]>0){
                $esta = "'".$se[1]."'";
                $obra = "'".$se[2]."'";
            ...
            $segui = mysql_query("select borrador, estado_cot_s, nombre_obra from seguimientos_cot where id_relacion='$cot' ");
            $se = mysql_fetch_array($segui);
            if($se[0]==0){
                $che = 'checked';
            }else{
                $che = '';  

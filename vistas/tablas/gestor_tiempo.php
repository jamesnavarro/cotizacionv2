<?php 
$request=mysql_query('select count(*) from cotizacion');
if($request){
...
if(isset($_POST['nombre']) || isset($_POST['fecha']) || isset($_POST['user'])){
  ...
  $request=mysql_query("SELECT * FROM cotizacion a where a.id_cliente like '%".$_POST['nombre']."%' and a.fecha_reg_c like '%".$fecha."%' and a.registrado like '%".$_POST['user']."%'");
}else{
   
        $request=mysql_query("SELECT * FROM cotizacion order by numero_cotizacion desc ");
}

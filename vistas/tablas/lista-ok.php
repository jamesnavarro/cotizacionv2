...
if(isset($_POST['buscar'])){
    ...
    $request=mysql_query("SELECT count(*) FROM producto where estado_producto=0 and ok=1 and concat(producto, ' ', codigo,' ',referencia_p) like '%".$_POST['b']."%' and linea like '%".$_POST['linea']."%' $id ");
    ...
    $request=mysql_query("SELECT count(*) FROM producto  where estado_producto=0 and ok=1 and concat(producto, ' ', codigo,' ',referencia_p) like '%".$_GET['b']."%' and linea like '%".$_GET['linea']."%' $id ");

}else{
    $request=mysql_query('select count(*) from producto where estado_producto=0 and ok=1 ');
}
}  
...
if(isset($_POST['buscar'])){
    ...
    $request=mysql_query("SELECT * FROM producto where estado_producto=0 and ok=1 and  concat(producto, ' ', codigo,' ',referencia_p) like '%".$_POST['b']."%' and linea like '%".$_POST['linea']."%' $id $rev $up  order by id_p asc ".$limit);
   ...
   if(isset($_GET['linea'])){
    ...
    $request=mysql_query("SELECT * FROM producto where estado_producto=0 and ok=1 and concat(producto, ' ', codigo,' ',referencia_p) like '%".$_GET['b']."%' and linea like '%".$_GET['linea']."%' $id order by id_p asc ".$limit);

}else{
    $request=mysql_query("SELECT * FROM producto where estado_producto=0 and ok=1 order by id_p asc ".$limit);
}
}  
	      ...
	      $num_items = 0;
              ...
              if($ver_conf == 'Habilitado'){
                    if($row['especial']==1){
                        ...
                        $x = '<a href="../vistas/?id=add_cot&cod='.$row['id_p'].'">'.$led2.'</a>';
                    }
                    ...
                    $cf =mysql_fetch_array(mysql_query($requestr));
                    $co = $cf['count(*)'];
                    if($co==0){

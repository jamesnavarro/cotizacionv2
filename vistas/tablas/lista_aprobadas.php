<?php
	...
	$request = mysql_query('SELECT COUNT(*) FROM cotizacion a, cont_terceros b WHERE a.id_tercero = b.id_ter AND a.estado = "Aprobado" ' . $cot . $nom . $obr . $reg . $ana );
	if ($request) {
...
<?php
	$request_ac = mysql_query("SELECT * FROM cotizacion a, cont_terceros b
								WHERE a.id_tercero = b.id_ter AND a.estado = 'Aprobado' " . $cot . $nom . $obr . $reg . $ana. "

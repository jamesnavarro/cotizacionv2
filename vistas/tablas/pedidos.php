...
if(isset($_POST['cadena'])){
    ...
    if($_GET['fecha1']=='' && $_GET['fecha2']==''){
       ...
       $request=mysql_query('select count(*) from cotizacion  where estado="Aprobado"');
    }}
if($request){
...
<?php
if(isset($_GET['pasar'])){
    $re=mysql_query("SELECT * FROM cotizacion ");
    $v = 0;
...
$request=mysql_query("SELECT *, (select concat(nombre,' ',apellido) from usuarios where id_user=aprobado_por) as nomb, (select sum(cant_restante) from cotizaciones where id_cot=a.id_cot) as cr FROM cotizacion a where a.estado='Aprobado' order by a.numero_cotizacion desc  ".$limit);

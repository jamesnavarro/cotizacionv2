...
echo $group;
if(isset($_GET['sig'])){
$request_ac=mysql_query('select *, (select opf from orden_produccion g where g.id_orden=e.id_op) as opfx from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where e.usuario='.$group.' and e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro  and b.id_subpro='.$id_area.' and  a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden');
}else{
$request_ac=mysql_query('select *, (select opf from orden_produccion g where g.id_orden=e.id_op) as opfx from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where e.usuario!=0 and e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro  and b.id_subpro='.$id_area.' and  a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden');
//$request_ac=mysql_query('select a.*, b.*, c.*, f.* from procesos_activos a, pt_procesos b, orden_detalle c, proceso d, subproceso e, orden_produccion f where f.id_orden=c.codigo and a.id_orden_d=c.id_orden_d and b.id_proceso=c.id_proceso and b.orden=a.paso and b.id_proceso=d.id_proceso and b.orden=a.paso and e.id_subpro='.$id_area.' and b.id_subpro='.$id_area.' group by a.id_orden_d');
}  
if($request_ac){
//    echo'<hr>';
 $table = '<table class="table table-bordered table-striped table-hover" id="">';
	...
	{       
               ...
               $rangos = mysql_query("select * from asignacion_grupo a, grupo b where a.id_g=b.id_g and a.id_area=".$id_area." and b.id_g=".$row['usuario']." ");

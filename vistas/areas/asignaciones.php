...
<div class="row-fluid">
    ...
    <form name="form1" class="" action="" method="post" enctype="multipart/form-data">
        ...
        <select name="grupox">
            ...
            <?php 
                $query = mysqli_query($conexion,"select * from asignacion_grupo a,grupo b where a.id_g=b.id_g and a.id_area=".$_GET['area']." ");
...
if(isset($_POST['buscar'])){
    $request=mysqli_query($conexion,'select *, (select opf from orden_produccion g where g.id_orden=e.id_op) as opfx from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where d.imprimir=0 and  e.id_op='.$_POST['op'].' and d.anula=0 and e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro  and b.id_subpro='.$valor1.' and  a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden');

}else{
    $request=mysqli_query($conexion,'select *, (select opf from orden_produccion g where g.id_orden=e.id_op) as opfx from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where d.imprimir=0 and d.anula=0 and e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro  and b.id_subpro='.$valor1.' and  a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden');
...
//    echo'<hr>';
 $table = '<table class="table table-bordered table-striped table-hover" id="">';
	...
	{      
        ...
        if($row['usuario']==0){
         $rangos = mysqli_query($conexion,"select * from asignacion_grupo a, grupo b where a.id_g=b.id_g and a.id_area=".$_GET['area']." ");
                $ran = '<input type="checkbox" name="c'.$c.'" class="span12">';
                
                $ran = $ran.'<input type="hidden" name="id'.$c.'" value="'.$row['id_proceso'].'">';
        }else{
           $rangos = mysqli_query($conexion,"select * from asignacion_grupo a, grupo b where a.id_g=b.id_g and a.id_area=".$_GET['area']." and b.id_g=".$row['usuario']." ");

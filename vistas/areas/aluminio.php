...
$res=mysql_query("SELECT * FROM subproceso where sede_sub='Aluminio'");
...
if(isset($_GET['prod'])){
    $consulta= 'select a.*, b.* from pt_procesos a, subproceso b where a.id_subpro=b.id_subpro and a.id_proceso='.$_GET['prod'].' order by a.orden asc ';  
      ...
      if(isset($_POST['area'])){
            $consulta= "SELECT * FROM subproceso where sede_sub='Aluminio' and id_subpro=".$_POST['area']." "; 
        }else{
    $consulta= "SELECT * FROM subproceso where sede_sub='Aluminio'"; 
        }
}                   
...
if(isset($_GET['orden_produccion'])){
    $request=mysql_query('select a.*, b.*, c.*, f.* from procesos_activos a, pt_procesos b, orden_detalle c, proceso d, subproceso e, orden_produccion f where e.save=1 and f.id_orden=c.codigo and a.id_orden_d=c.id_orden_d and b.id_proceso=c.id_proceso and b.orden=a.paso and b.id_proceso=d.id_proceso  and e.id_subpro='.$valor1.' and b.id_subpro='.$valor1.' and f.numero='.$_GET['orden_produccion'].'');

}else{
if(isset($_GET['orden'])){
    $request=mysql_query('select * from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro and b.id_subpro='.$valor1.'  and e.barra='.$_GET['orden'].' and a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden ');

}else{
    $request=mysql_query('select * from producto a, subproceso b, pt_procesos c, orden_detalle d, procesos_activos e where e.save=1 and a.id_p=c.id_proceso and c.id_subpro=b.id_subpro  and b.id_subpro='.$valor1.' and  a.id_p=d.id_producto and d.id_orden_d=e.id_orden_d and e.paso=c.orden');
...
//    echo'<hr>';
 $table = '<table class="table table-bordered table-striped table-hover" id="">';
	...
	{      
           $rangos = mysql_query("select * from asignacion_grupo a, grupo b where a.id_g=b.id_g and a.id_area=".$valor1." and b.id_g=".$row['usuario']." ");

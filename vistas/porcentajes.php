...
<script>
  ...
  <form name="buscarA" action="../vistas/porcentajes.php?cot=<?php echo $_GET['cot'] ?>&cli=<?php echo $_GET['cli'] ?>&por1" method="post" enctype="multipart/form-data">
      <input type="text" name="porcentaje1" placeholder="% por items">    <input  type="text" name="porcentaje2"  placeholder="% General">   Descuento Max <input  type="text" id="desc"  value="<?php echo $_GET['max'] ?>" disabled> 
      <input type="submit" name="buscar" value="Aplicar Incr/Desc" class="alt_btn">
      <a href="../vistas/porcentajes.php?cot=<?php echo $_GET['cot'] ?>&cli=<?php echo $_GET['cli'] ?>&salir"></form><button type="button"><img src="../imagenes/stop.png"> CERRAR</button></a>
        <hr>
  <form name="buscarA" action="../vistas/porcentajes.php?cot=<?php echo $_GET['cot'] ?>&cli=<?php echo $_GET['cli'] ?>&por" method="post" enctype="multipart/form-data">
...
//            echo ($tk ) .'<br>';
            ...
            $table = $table.'<tr><td>'.$tip.'</td><td>'.$tip2.'</td>
                    <td>'.substr($row['producto'], 0, 35).'</a></td>
                       <td>'.$row["ancho_c"].'x'.$row["alto_c"].'</font></td>
                             ...
                             <td><input type="hidden" name="sistema'.$cont.'" id="sistema'.$cont.'" style="width:70px" value="'.$pu.'">'
                    . '<input type="text" name="manual'.$cont.'" id="manual'.$cont.'" style="width:70px"></td>      
                   <td>$'.number_format($pu).'</font></td>
                       ...
                       <td>$'.number_format($ptt).'</font></td>
                            <td>$'.number_format($tota).'</font></td>
                                ...
                                <td width="90px">'.$select.'</a></td><td>'.number_format($POR).'%</a></td></tr>';   
	...
	echo $table.'<input type="hidden" name="cant" value="'.$cont.'">  <input type="submit" name="buscar" value="Actualizar" class="alt_btn">';
     ...
     echo '<div align="right"><FONT color="red"><h5>SUBTOTAL COT.: $<input style="width:80px;" type="text" value="'.number_format($total_por).'">';
...
//            mysqli_query($conexion,$sq);
            ...
            mysqli_query($conexion,$sqlm);
                  ...
                  mysqli_query($conexion,$sqlr);echo '<script lanquage="javascript">alert("Se ha Actualizado los porcentajes");location.href="../vistas/porcentajes.php?cot='.$_GET['cot'].'&cli='.$_GET['cli'].'"</script>'; 
}}
if(isset($_GET['por1'])){
            ...
            mysqli_query($conexion,$sqlm);
                  ...
                  $sqlr.= "VALUES ('Se Actualizaron los porcenjes de la cotizacion ', '".$_GET['cot']."', '".$_SESSION['k_username']."', 'Cotizacion')";
                  mysqli_query($conexion,$sqlr);
                  echo '<script lanquage="javascript">alert("Se ha Actualizado los porcentajes");location.href="../vistas/porcentajes.php?cot='.$_GET['cot'].'&cli='.$_GET['cli'].'"</script>'; 
}
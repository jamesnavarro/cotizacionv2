...
switch ($_GET['sw']){
    case 1:
        ...
        $idcot = $_GET['idcot'];
        $idpro = $_GET['idpro'];
        $result = mysqli_query($conexion,"select count(*), codigo from referencias where referencia='$ref' and medida='$per' ");
        $r = mysqli_fetch_array($result);
        $cod = $r[1];
        if($r[0]==0){
                     $query = mysqli_query($conexion,"insert into solicitudes_items (items,sistema,id_producto,id_cot_items,referencia_sc,descripcion_sc, codigo, color_sc, cantidad_sc, perfil_sc, id_cotizacion, registrado)"
    ...
    case 2:
        ...
        $per = $_GET['per'];
        $item = $_GET['item'];
        mysqli_query($conexion,"delete from solicitudes_items  where id_si='".$n[1]."' ");
    ...
    case 4:
             ...
             $request2=mysqli_query($conexion,'SELECT * FROM referencias WHERE codigo="'.$_GET['cod'].'"');
    ...
    case 5:
        ...
        $peso = $_GET['peso'];
        $area = $_GET['area'];
        mysqli_query($conexion,"update referencias set peso='$peso', area='$area' where codigo='$cod' ");
        
        break;
    case 6:
        $id = $_GET['id'];
        mysqli_query($conexion,"delete from solicitudes_items  where id_si='".$id."' ");
        break;
    case 7:
        $cot = $_GET['cot'];
        $peso = $_GET['peso'];
        ...
        $fecha=date("Y-m-d");
        if($sol==''){
            mysqli_query($conexion,"insert into solicitudes_diseno (numero_cotizacion, id_cot, solicitado, peso_total, area_total, fecha_registro, estado_sol, obs)"
                    . " values('$cot','$idcot','$user','$peso','$area','$fecha','Solicitado','$obs')");
            $ma = mysqli_query($conexion,"select max(id_sc) from solicitudes_diseno ");
            $m = mysqli_fetch_array($ma);
            $s = $m[0];
            mysqli_query($conexion,"update cotizacion set id_sc='$s' where id_cot='$idcot' ");
            mysqli_query($conexion,"update solicitudes_items set id_sc='$s' where id_cotizacion='$idcot' and id_sc=0 ");
        }else{
            mysqli_query($conexion,"update solicitudes_diseno set obs='$obs' where id_sc='$sol' ");
            $s = $sol;
        }
        echo $s;
        break;
        ...
        $cod = $_GET['cod'];
        $id = $_GET['id'];
        mysqli_query($conexion,"update solicitudes_items set codigo='$cod' where id_si='$id' ");
    ...
    case 9:
        ...
        $user = $_SESSION['k_username'];
        $color = $_GET['color'];
        $result = mysqli_query($conexion,"select count(*), codigo, perfil_sc, cantidad_sc,referencia_sc from solicitudes_items where referencia_sc='$ref' and mix!=0 ");
        $r = mysqli_fetch_array($result);
        $cod = $r[1];
        if($r[0]==0){
                 $p = array();
    ...
    case 10:
        ...
        case 11:
           ...
           $color = $_GET['color'];
                     ...
                     $query = mysqli_query($conexion,"insert into solicitudes_items (referencia_sc,descripcion_sc, codigo, color_sc, cantidad_sc, perfil_sc, id_cotizacion, registrado, mix)"
                    . "values ('$ref', '$des', '', '$color','$can', '$per', '$cot', '$user','1') ") ;
        ...
        case 12:
           ...
           $user = $_SESSION['k_username'];
           $color = $_GET['color'];
           $result = mysqli_query($conexion,"select codigo from referencias where referencia='$ref' and medida='$per' ");
           $r = mysqli_fetch_array($result);
           $cod = $r[0];
           $update = mysqli_query($conexion,"update solicitudes_items set cantidad_sc='$can', perfil_sc='$per', codigo='$cod' where id_cotizacion='$cot' and referencia_sc='$ref' and mix!=0 ");
        break;
        case 13:
           $cot = $_GET['cot'];
           $ref = $_GET['ref'];
           ...
           $r = mysqli_fetch_array($result);
           $cod = $r[0];
           $update = mysqli_query($conexion,"update solicitudes_items set cantidad_sc='$can', perfil_sc=perfil_sc-'$per', codigo='$cod' where id_cotizacion='$cot' and referencia_sc='$ref' and mix!=0 ");

           mysqli_query($conexion,"delete from solicitudes_items where perfil_sc=0 ");
        break;
         case 14:
           $per = $_GET['per'];
           $persta = $_GET['persta'];
        ...
        $cod = $r[1];
        if($r[0]==0){
                     $query = mysqli_query($conexion,"update solicitudes_items set perfil_sc='$per',codigo='' where referencia_sc='$ref' and perfil_sc='$persta' and id_cotizacion='$cot' ") ;
                 //echo $query;
           
        }else{
                    $query = mysqli_query($conexion,"update solicitudes_items set perfil_sc='$per',codigo='$cod' where  referencia_sc='$ref' and perfil_sc='$persta' and id_cotizacion='$cot' ") ;
               //echo $query;

...
<html>
    <head>
        <meta charset="UTF-8">
        <title>UTILIDAD DE BOGOTA CON COSTO CONTABLE PARA BOGOTA  No. <?php echo $num ?></title>
        <style type="text/css">
...
$nombr4=$f8['producto'];
 ...
 $compu =mysql_query("SELECT * FROM producto a, cotizaciones_sub c where c.id_referencia_sub=a.id_p and c.id_cot_sub=".$_GET["cot"]." and c.id_producto_cot=".$row["id_cotizacion"]."");
    ...
    $k =  (($row['precio_fom_sp'] +$costo_fom_mp)*1.1)*$row["cantidad_c"];
    $mas_porc = (($row['valor_fom'] + $k)/ $porcentaje_fom);

...
$pac=$f['pac'];
	...
	if($request){
		...
		$table = $table . '<thead >';
		$query_columnas_mano_obra = mysqli_query($conexion,"SELECT DISTINCT(c.referencia) FROM producto a, producto_rep_mo b, referencia_mo c, cotizaciones d WHERE b.id_ref_mo = c.id_ref_mo AND a.id_p = b.id_p AND d.id_referencia = a.id_p AND d.id_cot = " . $_GET['cot'] . " ORDER BY c.referencia");
		while ($row_columnas_mano_obra = mysqli_fetch_assoc($query_columnas_mano_obra)) {
		...
		$colspan = 24 + $i;
		$table = $table . '<tr BGCOLOR="#4E8CCF"><td colspan="' . $colspan . '" align="center">Descripción De Productos</td></tr>'
						. '<tr bgcolor="#D1EEF0">';
	...
	while ($row = mysqli_fetch_array($request)) {
		...
		$query_total_aluminio = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_alu b, referencias c WHERE b.id_ref_alum = c.id_referencia and a.id_p = b.id_p and a.id_p = " . $row['id_p']);
		$ta = 0;
		$ptt = 0;
		$tafom = 0;
		$vr1 = 0;
		...
		$vr7 = 0;
		$vr8 = 0;
		while ($row_total_aluminio = mysqli_fetch_assoc($query_total_aluminio)) {
		...
		$query_total_vidrios = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_vid b, referencias c WHERE b.id_ref_vid = c.id_referencia AND a.id_p = b.id_p AND a.id_p = " . $row["id_p"]);
		$total_vid = 0;
		$total_vidsp = 0;
		$cu = 0;
		$peso_vid = 0;
		...
		$altura_v_c = 0;
		$anchura_v_c = 0;
		while ($row_total_vidrios = mysqli_fetch_array($query_total_vidrios)) {
			$ci += 1;
			if ($row_total_vidrios["ancho_v"] == 0) {
				...
				$fil_an = mysqli_fetch_array(mysqli_query($conexion,$query_total_x_vidrios));
				$id_p = $fil_an["id_p"];
		...
		$query_total_rejilla = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_rej b, referencias c WHERE b.id_referencia = c.id_referencia AND a.id_p = b.id_p AND a.id_p = " . $row['id_p']);
		$xx = 0;
		$xxfom = 0;
		$peso_rej = 0;
		while ($row_total_rejilla = mysqli_fetch_assoc($query_total_rejilla)) {
			...
			$query_total_v_rejilla = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_vid b, referencias c WHERE b.id_ref_vid = c.id_referencia AND a.id_p = b.id_p AND a.id_p = " . $row['id_p'] . " AND b.id_r_v = " . $row_total_rejilla["medida_rej"] . " ");
			while ($row_total_v_rejilla = mysqli_fetch_array($query_total_v_rejilla)){
		...
		$query_total_accesorios = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_acc b, referencias c WHERE b.id_ref_acc = c.id_referencia AND a.id_p = b.id_p and a.id_p = " . $row['id_p'] . " ORDER BY b.para");
		$total2 = 0;
		$tac = 0;
		$tacfom = 0;
		$peso_acc = 0;
		$rf5['pelicula'];
		while($row_total_accesorio = mysqli_fetch_array($query_total_accesorios)) {
			if ($row_total_accesorio['can_rej'] != 0) {
				$query_total_v2_accesorio = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_rej b, referencias c WHERE b.id_referencia = c.id_referencia AND a.id_p = b.id_p AND a.id_p = " . $row['id_p'] . " AND b.id_r_rej = " . $row_total_accesorio['can_rej'] . " ");
				while ($row_total_v2_accesorio = mysqli_fetch_array($query_total_v2_accesorio)) {
					...
					$fil_an = mysqli_fetch_array(mysqli_query($conexion,$query_total_xy_accesorio));
					$id_p = $fil_an["id_p"];
					if ($fil_an['signo'] == '+') {
						if ($fil_an['medida_r_a'] == 1) {
							$al = ($altura_v_c + $fil_an["descuento"]) + $fil_an['variable'];
		...
		$query_total_gastos_maquinaria = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_ma b, referencia_ma c WHERE b.id_ref_ma = c.id_ref_ma AND a.id_p = b.id_p AND a.id_p = " . $row['id_p']);
		$tot2 = 0;
		$tot2_p = 0;
		$tot2fom = 0;
		$tot2_pfom = 0;
		while ($row_total_gastos_maquinaria = mysqli_fetch_array($query_total_gastos_maquinaria)) {
		...
		$query_total_mano_obra = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_mo b, referencia_mo c WHERE b.id_ref_mo = c.id_ref_mo AND a.id_p = b.id_p AND a.id_p = " . $row["id_p"]);
		$tot = 0;
		$tot_fom = 0;
		while ($row_total_mano_obra = mysqli_fetch_assoc($query_total_mano_obra)) {
			$mt2 = ($row['alto_c'] / 1000) * ($row['ancho_c'] / 1000);
		...
		$query_total_gastos_administrativos = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_ad b, referencia_admin c WHERE b.id_ref_ad = c.id_ref_ad AND a.id_p = b.id_p AND a.id_p = " . $row["id_p"]);
		$tota=0;
		$tota_p=0;
		$totafom=0;
		$tota_pfom=0;
		while ($row_total_gastos_administrativos = mysqli_fetch_array($query_total_gastos_administrativos)) {
			$por = 100;
...
<td class="hidden-phone">$ ' . number_format($vr4) . '</td>
<td class="hidden-phone">$ ' . number_format($total_vid * $row["cantidad_c"]) . '</td>
<td class="hidden-phone">$ ' . number_format($xx) . '</td>
<td class="hidden-phone">$ ' . number_format($tac) . '</td>
<td class="hidden-phone">$ ' . number_format($tot2_p) . '</td>';
		foreach ($mano_obra as $valor_mano_obra) {
			$query_valor_mano_obra = mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_mo b, referencia_mo c, cotizaciones d WHERE b.id_ref_mo = c.id_ref_mo AND a.id_p = b.id_p AND d.id_referencia = a.id_p AND d.id_cot = " . $_GET['cot'] . " AND a.id_p = " . $row["id_p"] . " AND c.referencia = '" . $valor_mano_obra . "'");
			$row_valor_mano_obra = mysqli_fetch_array($query_valor_mano_obra);
			if ($row_valor_mano_obra['referencia'] != "") {
				if ($row_valor_mano_obra['referencia'] != "26002") {
					...
					if ($row_valor_mano_obra['unidad_cobro'] == 'M2') {
						$table = $table . '<td class="hidden-phone">$ ' . number_format($mt2 * ($row_valor_mano_obra["valor_mo"]) * $row['cantidad_c']) . '</th>';
					}
					if ($row_valor_mano_obra['unidad_cobro'] == 'ML') {
						$table = $table . '<td class="hidden-phone">$ ' . number_format($mtl * ($row_valor_mano_obra["valor_mo"]) * $row['cantidad_c']) . '</th>';
					}
					if ($row_valor_mano_obra['unidad_cobro'] == 'Und') {
						$table = $table . '<td class="hidden-phone">$ ' . number_format(($row_valor_mano_obra["valor_mo"]) * $row['cantidad_c']) . '</th>';
					}
...
$table = $table . '<td class="hidden-phone">$ ' . number_format($tota_p) . '</td>';

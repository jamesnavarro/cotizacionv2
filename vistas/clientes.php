...
function guardar_cliente(id){
    ...
    if(ema==''){
     ...
     if(telefono==''){
      ...
      $.ajax({
            type:'GET',
            data:'cedula='+cedula+'&ver='+ver+'&nombre='+nombre+'&tipo='+tipo+'&direccion='+direccion+'&ciudad='+ciudad+'&municipio='+municipio+'&telefono='+telefono+'&celular='+celular+'&ema='+ema+'&contacto='+contacto+'&desv='+desv+'&idc='+idc+'&clint_ase='+clint_ase+'&est='+est+'&sw=1',
            url:'../vistas/terceros/acciones.php',
            success : function(t){
                var p = eval(t);
                 alert(p[0]);
...
function buscarclifom(){
    var cod = $("#cedula").val();
        if(cod==''){
            alert("Debes de digitar el numero de documento");
            return false;
        }
      $.ajax({
          type:'GET',
          ...
          dataType: 'json',
          success: function(da){
                if(da.CLI_ACTIVO==false){
                    
                
                $("#nombre").val(da.CLI_NOMBRE);
                var tipo = $("#tipo").val();
                var direccion = $("#direccion").val(da.CLI_DIRECC);
                //var ciudad = $("#ciudad").val(da.CLI_ZONA);
                //var municipio = $("#municipio").val(da.CLI_SUBZON);
                var telefono = $("#telefono").val(da.CLI_TELEFO);
                var celular = $("#celular").val(da.CLI_AEREO);
                var celular = $("#ema").val(da.CLI_EMAIL);
                buscar_ciudad(da.CLI_ZONA);
                buscar_municipio(da.CLI_SUBZON);
            }else{
                alert("Este cliente se encuentra inactivo");
                return false;
            }
          }
      }).fail( function( jqXHR, textStatus, errorThrown ) {
             alert( 'Este Cliente no se encuentra en fomplus');
//           $("#est"+ced).html('');
             return false;
        });
  }
  function buscar_ciudad(cod){
      $.ajax({
            type:'GET',
            data:'cod='+cod+'&sw=4',
            url:'../vistas/terceros/acciones.php',
                        ...
                        <div class="span12 widget lime">
                            ...
                            <section class="body">
                                <div class="body-inner padding">
                                    <table style="width:100%">
                                        <tr>
                                            ...
                                            <td><input type="text" autocomplete="off"  id="cedula" value="" class="span6" maxlength="18" >
                                                <button type="button" onclick="buscarclifom()">Buscar en Fom</button>
                                                <input type="hidden"  id="ver" value="" style="width:40px" ></td>
                               ...
                               ?>      
                                        ...
                                        <tr>
                                             <td><label class="control-label">Estado del cliente</label></td>
                                            <td>
                                                <select  id="est" >
                                                    <option label="" value="0">Activo</option>
                                                    <option label="" value="1">No Activo</option>

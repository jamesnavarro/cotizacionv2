...
<?php
	...
	if (isset($_GET['cot'])) {
    	$request = mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c WHERE c.id_referencia = a.id_p AND c.servicio = 0 AND c.id_cot = " . $_GET["cot"] . " ORDER BY c.fila ASC");
		if ($request) {
...
if($row['msg']!=''){$noti='<br><b> <font color="red">'.$row['msg'].' </b>';}else{$noti='';}
    ...
    $table3 = $table3.'<tr>'
                    . '<td width="4%"  height="20" style="font-size:8px"><p align="center">'.$row['tip'].'</p></td>          
                       <td width="23%" style="font-size:8px"><p align="justify">  '.strtoupper($row['producto']).' '.$esp.' '.strtoupper($obs).' '.strtoupper($v).' '.strtoupper($peli).'  '.strtoupper($d1).'  '.strtoupper($obs2).'  '.$noti.''.$noti2.'</p></td>

...
if($_POST['perimetro']==0){
    $peri = ' and area=0 ' ;
}else{
    $peri = ' ' ;
}
$request=mysqli_query($conexion,"select referencia from referencias where concat(codigo,' ', descripcion,' ', referencia) like '%".$_POST['buscador']."%'  and grupo like '".$_POST['grupo']."%' $peri $grupo  ");
...
<?php
    ...
    $limit = 'LIMIT ' .($page - 1) * $rows_by_page .',' .$rows_by_page;
        ...
        . "grupo like '%".$_POST['grupo']."' $peri $grupo  order by id_referencia desc ".$limit;
        ...
        <table class="table table-bordered table-striped table-hover">
            <thead>
                <tr bgcolor="#D1EEF0">
                    ...
                    <th class="hidden-phone">Moneda</th>
                    <th class="hidden-phone">Costo COP</th>
                    <th class="hidden-phone">Costo USD</th>
	...
	{       
            ...
            if($ver_conf=='Habilitado'){$ver3='<center><button><img src="../imagenes/hoja.png"></button></center>';}else{$ver3='';}        
               IF($row["grupo"]=='Perfileria'){
                   $dis = '';
                   $funcion = '../vistas/ver_referencia.php?id_refe=';
               }ELSE{
                   $dis = 'disabled';
                   $funcion = '../vistas/vidrio/historial.php?tipo=Accesorios&id=';
               }
        ?>
                <tr>
                    <td width="5%"><?php echo $row['id_referencia']; ?></td>
                    <td width="5%"><?php echo $row['codigo']; ?></font></td>
                    <td width="30%"><?php echo$row['descripcion']; ?><br>Mod:<?php echo$row['modificado']; ?><br><?php echo$row['fecha_modificado']; ?></td>
                    ...
                    <td width="10%"><?php echo $row["moneda"]; ?><font></a></td>
                    <td width="10%">$<?php echo number_format($row["costo_mt"]); ?><font></a></td>
                    <td width="10%">$<?php echo number_format($row["costo_usd"],2); ?><font></a></td>
                    ...
                    <td><a href="<?php echo $funcion.$row["id_referencia"]; ?>" target="_blank" onClick="window.open(this.href, 'form', 'width=800,height=800'); return false;"><?php echo $ver2; ?></a> </td>

...
//                    . '<td><a href="javascript:void(0)" onclick="up_desglose('.$row["id_desglose"].','.$_GET['cod'].');"><button>Editar</button></a></td>'
       ...
       while($col=mysqli_fetch_array($request_vrej))
	{
        ...
        $tac =$tac6;
                 ...
                 $requestcomp=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones_sub c  where c.id_referencia_sub=a.id_p and c.id_producto_cot=".$idcot."");
                 $tac3 = $tac6;
                 while($rowcomp=mysqli_fetch_array($requestcomp))
	         {

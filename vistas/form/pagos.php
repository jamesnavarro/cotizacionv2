<?php 
if(isset($_GET['cod'])){
    $query = mysqli_query($conexion,"SELECT * FROM pagos where id_pago=".$_GET['cod']." ");
...
$request_ac=mysqli_query($conexion,"SELECT * FROM pagos ");
...
//    echo'<hr>';
	...
	{           if($row['estado_p']==0){$im = '<img src="../imagenes/ok.png">';$e = 'yes';}else{$im = '<img src="../imagenes/cancelar.png">';$e = 'no';}
              if($crear_conf=='Habilitado'){$add='<img src="../imagenes/empleado.png">';}else{$add='';}
                ...
                $t = $t +1;
                $rangos = mysqli_query($conexion,"select * from pagos_rangos where id_pago=".$row['id_pago']." ");
                $ran = '';

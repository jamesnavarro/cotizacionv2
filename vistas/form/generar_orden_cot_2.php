...
//include '../vistas/imprimir_reportes.php';
	...
	$query_total_item = mysql_query("SELECT * FROM producto a, cotizaciones c where c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]." and c.id_compuesto=0  order by c.fila asc");
...
<!--                                        <button><a target="_blank" href="../vistas/?id=send&cot=<?php echo $_GET['cot']; ?>&cli=<?php echo $_GET['cli'].'&tipo='.$tipo.' '; ?>"><img src="../imagenes/carta.jpg"> Enviar Email</a></button>-->
            ...
            $sql21 = ("SELECT *, a.rieles as cherieles,a.alfajia as chealfajia, a.cierres as checierres, a.rodajas as cherodajas   FROM producto a, cotizaciones c where a.id_p=c.id_referencia and c.id_cotizacion='".$_GET['up']."' ");
            $fila21 =mysql_fetch_array(mysql_query($sql21));
            ...
            $result_rieles = mysql_query("SELECT descripcion FROM referencias   where id_referencia='$rieles' ");
            $rie = mysql_fetch_array($result_rieles);
            $desrieles = $rie[0];
            
            $result_alfa = mysql_query("SELECT descripcion FROM referencias   where id_referencia='$alfajia' ");
            $alf = mysql_fetch_array($result_alfa);
            $desalfajias = $alf[0]; 
            
            $result_cie = mysql_query("SELECT descripcion FROM receta_kits   where idkit='$cierres' ");
            $cie = mysql_fetch_array($result_cie);
            $descierres = $cie[0];
            
            $result_rod = mysql_query("SELECT descripcion FROM receta_kits   where idkit='$rodajas' ");
            $rod = mysql_fetch_array($result_rod);
            $desrodajas = $rod[0];
                                     ...
                                     <table class="table table-bordered table-striped table-hover">
                                        <tr>
                                            ...
                                            <td> <select name="install"  style="width: 80px;">
                                                        ...
                                                        <td rowspan="9" id="imagen"  style="width:30%">
                                                            <div align="center"><?php if(isset($_GET['up'])){
                                                if($lado=='Derecho'){
                                            ...
                                            <td>
                                                <select id="idesp" name="idesp">
                                                     ...
                                                     $query2eup= "SELECT * FROM receta_kits where idkit='$idespaciador' ";                     
                                                            $resul2eup=  mysql_query($query2eup);
                                                            $fila2=  mysql_fetch_array($resul2eup);
                                                           ...
                                                           $query2e= "SELECT * FROM receta_kits where modulo like 'Espaciadores' and estado='ACTIVO' and idkit!='$idespaciador' ";                     
                                                            $resul2e=  mysql_query($query2e);
                                                            while($fila=  mysql_fetch_array($resul2e)){
                                                            $valor1=$fila['idkit'];
                ...
                while($cpt = mysql_fetch_array($com)){
                    ...
                    $compuesto = $compuesto.'<li>'.$cpt['producto'].' Med: '.$cpt['ancho_c_sub'].'x'.$cpt['alto_c_sub'].'</li>';
                        ...
                        <!--/ END Form Wizard -->
                                ...
                                <button onclick="reporte_itemsxd(<?php echo $_GET['cot'] ?>)" type="button" id="btn_report">Generar Reporte de Costo Materia Prima</button>
                                <span id="rep">
                                </span>
...
//                                        $gen_desc = $pg[0] / 100;
            ...
            $comp=mysql_query("SELECT count(*) FROM cotizaciones_sub  where id_producto_cot=".$row["id_cotizacion"]."");
            $cm1 = mysql_fetch_array($comp);
            $d = $cm1['count(*)'];
            
            $ac =mysql_query("SELECT  count(*) FROM referencia_acce  where id_cot=".$row["id_cotizacion"]."  ");
            $cm = mysql_fetch_array($ac);
            $mt = $cm['count(*)'];
            
              $ak =mysql_query("SELECT count(*) FROM cotizaciones_kit  where id_prod_mas=".$row["id_cotizacion"]." and comp='1'  ");
            $ck = mysql_fetch_array($ak);
            $mtk = $ck['count(*)'];
            
            $as =mysql_query("SELECT count(*) FROM cotizaciones_servicios  where id_cot_mas=".$row["id_cotizacion"]." ");
            $cs = mysql_fetch_array($as);
            $mts = $cs['count(*)'];
            
            $asn =mysql_query("SELECT count(*) FROM cotizaciones  where id_compuesto=".$row["id_cotizacion"]." ");
            $csn = mysql_fetch_array($asn);
            $cop = $csn['count(*)'];
            
            $compu =mysql_query("SELECT * FROM  cotizaciones_sub  where id_producto_cot=".$row["id_cotizacion"]."");
	...
	echo $table4;
        echo '<div align="right"><FONT color="red"><h5>TOTAL KIT.: $'.number_format($to_k).'</h5></FONT></div>';
         //echo '<div align="right"><FONT color="red"><h4>GRAN TOTAL.: $'.number_format($to_serv+$tacot+$to_mat+$to_k).'</h4></FONT></div>';
          ...
          mysql_query("update seguimientos_cot set vtotal_obra='$gt_total' where id_relacion='".$_GET["cot"]."' ");
          if($estado!='Aprobado'){                             
          ?>
          <table>
             <tr>
                            ...
                            <section class="body">
                                <div class="body-inner">
                                    ...
                                    Peso: <input type="text" id="pesov" name="pesov" value="0Kgs" readonly  style="width:20px" > 
                                     ...
                                     %Ace: <input type="text" id="dalu" name="despace" value="20" style="width:20px" >
                                                    ...
                                                    <option value="">Seleccione</option>
                                                     <?php         
                                                           $query2= "SELECT * FROM receta_kits where modulo like 'Espaciadores' and estado='ACTIVO' ";                     
                                                            $resul2=  mysql_query($query2);
                                                            while($fila=  mysql_fetch_array($resul2)){
                                                            $valor1=$fila['idkit'];

...
//                mysql_query($sqlo);
 ...
 mysql_query("UPDATE `seguimientos_cot` SET `estado_cot_s` = 'Adjudicado' WHERE `id_relacion` ='".$_GET['cot']."' ");
...
echo '<script lanquage="javascript">alert("Se ha generado el pedido");location.href="../vistas/?id=new_fac&cot='.$_GET['cot'].'&cli='.$_GET['cli'].'"</script>'; 
    ...
    $sqlxi = "SELECT * FROM ivas";
            $fi =mysql_fetch_array(mysql_query($sqlxi));
            $iva= $fi["iva"];
 ?>
            ...
            <section class="body">      
                        ...
                        <header>
                         <h4 class="title">Ordenes de Produccion</h4>
                                        ...
                                        <?php
$request_ac=mysql_query('select * from orden_produccion where ref='.$_GET['cot'].' order by id_orden desc ');

...
.datagrid table { border-collapse: collapse; text-align: left; width: 100%; } .datagrid {font: normal 12px/150% Arial, Helvetica, sans-serif; background: #fff; overflow: hidden; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; }.datagrid table td, .datagrid table th { padding: 3px 10px; }.datagrid table thead th {background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #006699), color-stop(1, #00557F) );background:-moz-linear-gradient( center top, #006699 5%, #00557F 100% );filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#006699', endColorstr='#00557F');background-color:#006699; color:#FFFFFF; font-size: 12px; font-weight: bold; border-left: 0px solid #0070A8; } .datagrid table thead th:first-child { border: none; }.datagrid table tbody td { color: #00496B; border-left: 2px solid #E1EEF4;font-size: 12px;border-bottom: 2px solid #E1EEF4;font-weight: normal; }.datagrid table tbody td:first-child { border-left: none; }.datagrid table tbody tr:last-child td { border-bottom: none; }.datagrid table tfoot td div { border-top: 1px solid #006699;background: #E1EEF4;} .datagrid table tfoot td { padding: 0; font-size: 12px } .datagrid table tfoot td div{ padding: 2px; }.datagrid table tfoot td ul { margin: 0; padding:0; list-style: none; text-align: right; }.datagrid table tfoot  li { display: inline; }.datagrid table tfoot li a { text-decoration: none; display: inline-block;  padding: 2px 8px; margin: 1px;color: #FFFFFF;border: 1px solid #006699;-webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #006699), color-stop(1, #00557F) );background:-moz-linear-gradient( center top, #006699 5%, #00557F 100% );filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#006699', endColorstr='#00557F');background-color:#006699; }.datagrid table tfoot ul.active, .datagrid table tfoot ul a:hover { text-decoration: none;border-color: #006699; color: #FFFFFF; background: none; background-color:#00557F;}div.dhtmlx_window_active, div.dhx_modal_cover_dv { position: fixed !important; } </style>
        ...
        function planilla(cot,gt){
            ...
            window.open("../../vistas/costos/planilla_costo.php?cot="+cot+"&t="+t+"&t_alu="+t_alu+"&t_ace="+t_ace+"&t_vid="+t_vid+"&t_acc="+t_acc+"&t_adi="+t_adi+"&t_kit="+t_kit+"&t_mob="+t_mob+"&t_ins="+t_ins+"&t_pol="+t_pol+"&t_and="+t_and+"&gt="+gt+"&item=0 ", "Planilla", "width=1000 , height=600 ");
        }
        function planilla2(cot,gt){
            var t_alu = $("#t_alu").val();
            var t_ace = $("#t_ace").val();
            var t_vid = $("#t_vid").val();
            var t_acc = $("#t_acc").val();
            ...
            t_acc = parseFloat(t_acc);
            t_ins =  parseFloat(t_ins);
            window.open("../../vistas/costos/planilla_costo.php?cot="+cot+"&t="+t+"&t_alu="+t_alu+"&t_ace="+t_ace+"&t_vid="+t_vid+"&t_acc="+t_acc+"&t_adi="+t_adi+"&t_kit="+t_kit+"&t_mob="+t_mob+"&t_ins="+t_ins+"&t_pol="+t_pol+"&t_and="+t_and+"&gt="+gt+"&item=0&reporte ", "Planilla", "width=1000 , height=600 ");
        }
        </script>
    ...
    <body>
        ...
        ?>
        <div class="datagrid"  style="width:1000px; height:600px; overflow: scroll;">
            <span id="btn"> 
        ...
        <table>
                ...
                while($row=mysqli_fetch_array($request))
	        {
                    if($row['ancho_temp']=='0'){
                        ...
                        $ancho = $row['ancho_temp'];
                        $alto = $row['alto_temp'];
                    }
                    $traz_vid= $row["traz_vid"];
                    $traz_vid2= $row["traz_vid2"];
                    $traz_vid3= $row["traz_vid3"];
                    $traz_vid4= $row["traz_vid4"];
                    
                    $id_vidrio= $row["id_vidrio"];
                    $id_vidrio2= $row["id_vidrio2"];
                    $id_vidrio3= $row["id_vidrio3"];
                    $id_vidrio4= $row["id_vidrio4"];
                    $id_vidrio5= $row["id_vidrio5"];
                    $id_vidrio6= $row["id_vidrio6"];
                    $pelicula= $row["pelicula"];
                    $id2_vidrio= $row["id2_vidrio"];
                    $id2_vidrio2= $row["id2_vidrio2"];
                    $id2_vidrio3= $row["id2_vidrio3"];
                    $id2_vidrio4= $row["id2_vidrio4"];
                    $id2_vidrio5= $row["id2_vidrio5"];
                    $id3_vidrio= $row["id3_vidrio"];
                    $id3_vidrio2= $row["id3_vidrio2"];
                    $id3_vidrio3= $row["id3_vidrio3"];
                    $id3_vidrio4= $row["id3_vidrio4"];
                    $id3_vidrio5= $row["id3_vidrio5"];
                    $id4_vidrio= $row["id4_vidrio"];
                    $id4_vidrio2= $row["id4_vidrio2"];
                    $id4_vidrio3= $row["id4_vidrio3"];
                    $id4_vidrio4= $row["id4_vidrio4"];
                    $id4_vidrio5= $row["id4_vidrio5"];
                    $descripcion_vidrio = '';
                    if($traz_vid!=0){
                    ...
                    $descpor = $row['precio_item'] *  $row["desc"]/100;
                    $totalitem=$row['precio_item']+$descpor;

                        
                    echo '<tr>';
                    echo '<td>'.$row['tip'].'</td>';
                    echo '<td>'.$row['producto'].' '.$row['observaciones'].' '.$row['observaciones2'].' '.$row['color_ta'].'</td>';
                    echo '<td>'.$descripcion_vidrio.'</td>';
                    echo '<td>'.$ancho.'</td>';
                    echo '<td>'.$alto.'</td>';
                    echo '<td>'.$row['cantidad_c'].'</td>';
                    echo '<td>'.$row['desc'].'%</td>';
                     echo '<td>'.number_format($totalitem/$row['cantidad_c']).'</td>';
                    echo '<td>'.number_format($totalitem).'</td>';
                    $query_alu = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes,ifnull(sum(peso_totales),0) from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Aluminio','Rejillas') ");
                    $a = mysqli_fetch_row($query_alu);
                    echo '<td style="text-align:right">'.number_format($a[0]).'</td><td style="text-align:right">'.number_format($a[2]).'</td>';
                    $query_vid = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes,ifnull(sum(peso_totales),0) from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Vidrio') ");
                    ...
                    $query_acc = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes,ifnull(sum(peso_totales),0) from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Accesorios') ");
                    $ac = mysqli_fetch_row($query_acc);
                    echo '<td style="text-align:right">*'.number_format($ac[0]).'</td>';
                     $query_ad = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes,ifnull(sum(peso_totales),0) from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Adicional') ");
                    $ad = mysqli_fetch_row($query_ad);
                    echo '<td style="text-align:right">'.number_format($ad[0]).'</td><td style="text-align:right">'.number_format($ad[2]+$ac[2]).'</td>';
                     $query_ace = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes,ifnull(sum(peso_totales),0) from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Acero') ");
                    $ce = mysqli_fetch_row($query_ace);
                    echo '<td style="text-align:right">'.number_format($ce[0]).'</td><td style="text-align:right">'.number_format($ce[2]).'</td>';
                    
                    $query_ki = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Kits') ");
                    $ki = mysqli_fetch_row($query_ki);
                    echo '<td style="text-align:right">'.number_format($ki[0]).'</td>';
                    $query_mo = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Fabricacion') ");
                    $mo = mysqli_fetch_row($query_mo);
                    echo '<td style="text-align:right">'.number_format($mo[0]).'</td>';
                    $query_in = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Instalacion') ");
                    $i = mysqli_fetch_row($query_in);
                    echo '<td style="text-align:right">'.number_format($i[0]).'</td>';
                    $query_p = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Polimask') ");
                    $p = mysqli_fetch_row($query_p);
                    echo '<td style="text-align:right">'.number_format($p[0]).'</td>';
                    $query_an = mysqli_query($conexion,"select ifnull(sum(costo_totales),0),porcentajes from costo_totales where id_cot=".$_GET["cot"]." and (id_cotizaciones='".$row['id_cotizacion']."' or id_cotizacion_mas='".$row['id_cotizacion']."') and tipo_costo in ('Andamios') ");
            ...
            ?>
                    <tr>
                        ...
                        <td style="text-align:right"><?php echo number_format($p_acc) ?><input type="hidden" id="p_acc" value="<?php echo ($p_acc) ?>"> Kg </td>
                         <td style="text-align:right"><?php echo number_format($t_ace) ?><input type="hidden" id="t_ace" value="<?php echo ($t_ace) ?>"></td>
                         <td style="text-align:right"><?php echo number_format($p_ace,2) ?><input type="hidden" id="p_acc" value="<?php echo ($p_ace) ?>"> Kg </td>
                    ...
                    <tr>
                        <tr>
                        <td colspan="20">Total Costo Directo
                             
                            <?php if(isset($_GET['reporte'])){  ?>
                                <button onclick="planilla2(<?php echo $_GET['cot']; ?>,<?php echo $_GET['gt']; ?>)">Planilla de Costo</button>
                            <?php }else{ ?>
                        ...
                        <td style="text-align:right">
                            <?php echo number_format($t_and+$t_pol+$t_alu+$t_ins+$t_vid+$t_acc+$t_adi+$t_kit+$t_mob+$t_ace) ?>
                            <input type="hidden" id="totales" value="<?php echo ($t_and+$t_pol+$t_alu+$t_ins+$t_vid+$t_acc+$t_adi+$t_kit+$t_mob+$t_ace) ?>">
                        </td>
                    <tr>
                        <td colspan="20">Peso Total 

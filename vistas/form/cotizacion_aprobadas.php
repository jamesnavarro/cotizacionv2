...
echo '<script lanquage="javascript">alert("Se ha generado el pedido");location.href="../vistas/?id=new_fac_aprobadas_aprobadas&cot='.$_GET['cot'].'&cli='.$_GET['cli'].'"</script>'; 
...
function aprobar_item(act, id){
    $.ajax({
        type: 'GET',
        data: 'id='+id+'&sw='+act,
        url: '../vistas/aprobadas/acciones.php',
    ...
    <button type="button" id="detallecot2" style="display: none">Ocultar Encabezado Cotizacion</button>
                        ...
                        <section class="body" id="detallecoti">
                                ...
                                <div class="control-group">
                                    <form class="span12 widget shadowed dark form-horizontal bordered" action="<?php if(isset($_GET['update'])){echo '../vistas/?id=new_fac_aprobadas_aprobadas&cot='.$_GET['cot'].'&cli='.$_GET['cli'].'&editar';}else{echo '../vistas/?id=new_fac_aprobadas_aprobadas&consultar';} ?>" method="post" id="form_validate_html" enctype="multipart/form-data">
                                                ...
                                                echo 'Cotizacion No.(<font color="white">V.'.$numero_cotizacion.'.'.$version.'</font>)';}else{
                                                    ...
                                                    if(isset($_GET['cot'])){
                                                        ...
                                                        if($editar_cot == 'Habilitado'){
                                                            ...
                                                            ?></h4> <a href="<?php echo '../vistas/?id=new_fac_aprobadas_aprobadas&cot='.$_GET['cot'].'&cli='.$_GET['cli'].'&update ';  ?>"><button type="button"><img title="Editar cliente de la cotizacion" src="../imagenes/modificar.png"> Editar</button></a>
                                                        <?php }}}
                                                        if(isset($_GET['update'])){
    ...
    <input type="text" id="vendedo" required autocomplete="off" name="asesor"  onClick="window.open('../vistas/asesor.php', 'buscar', 'width=800,height=800'); return false;"  value="<?php if (isset($_GET['cot'])) {echo $asesor ;} ?>"  placeholder="" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?>>
            ...
            <div class="span6">
                <header><h4 class="title">Datos de la Obra</h4></header>
                                            ...
                                            <div class="controls"><input type="text" name="obra" autocomplete="off" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?>   value="<?php  if(isset($_GET['cot'])){echo $obra;}  ?>" ></div>
                 <label></label><div class="controls"> </div>
      ...
      while($fila=  mysql_fetch_array($result)){       
          ...
          ?>       
           ...
           <label></label><div class="controls"> </div>
                                            ...
                                            <div class="controls"><input type="text" autocomplete="off" name="pago" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?>  value="<?php if (isset($_GET['cot'])) {echo $pago ;} ?>"  placeholder="" ></div>
                     ---------------------------------------------------(Temporal)----------------------------------
<label></label><div class="controls"> </div>
<label class="control-label">Cedula / Nit.</label>   
<div class="controls">
    <input type="text" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?> autocomplete="off" name="cod_temp"  value="<?php  if(isset($_GET['cot'])){echo $cod_temp;}  ?>"  placeholder="" >
</div>

<label></label><div class="controls"> </div>
<label class="control-label">Nombre del Cliente</label>
<div class="controls">
    <input type="text" autocomplete="off" name="nombre_temp" value="<?php  if(isset($_GET['cot'])){echo $nom_temp;}  ?>"  <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?> placeholder="" >
</div>
...
<div class="controls">
	<input name="fecha_serv_express" class="span5" type="text" id="datepicker1" placeholder="<?php echo date("Y-m-d"); ?>" value="<?php  if(isset($_GET['cot'])){echo $fecha_express;}  ?>" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?> />
                                            ...
                                            <div class="controls"><textarea name="nota" style="width: 100%;" <?php if(!isset($_GET['update']) && isset($_GET['cot'])){echo 'readonly';} ?> class="span12"><?php if(isset($_GET['cot'])){echo $nota;}  ?></textarea></div>
 ...
 ?> 
                                    ...
                                    if(isset($_GET['consultar'])){
                                            ...
                                            $sql = "INSERT INTO `cotizacion` (`express`, `fecha_de_entrega`, `nota`, `fecha_modificacion`, `fecha_reg_c`, `pago`, `registrado`, `version`, `numero_cotizacion`,`presupuesto`,`tipo`, `instalacion`,`precio`,`aiu`,`responsable`,`tel_responsable`,`ciudad`,`municipio`,`id_tercero`, `grabado`, `estado`, `obra`, `ubicacion`, `linea`,`validez`,`cod_temp`,`nom_temp`)";
                                            $sql.= "VALUES ('" . $_POST['serv_express'] . "', '" . $fecha_serv_express . "', '" . $_POST['nota']."','".$fecha_hoy."', '".$fecha_hoy."', '".$_POST['pago']."', '".$_POST['asesor']."', '1', '".$max_nc."', '".$_POST['presupuesto']."','".$_POST['tipo']."','Si','p1','No','".$_POST['encargado']."','".$_POST['tel_o']."','".$_POST['departamento']."','".$_POST['municipio']."','".$_POST['id_cli']."', '".$_SESSION['k_username']."', 'En proceso', '".$_POST['obra']."', '".$_POST['ubicacion']."', 'Aluminio','".$_POST['validez']."','".$_POST['cod_temp']."','".$_POST['nombre_temp']."')";
                                            mysql_query($sql, $conexion);
                                            ...
                                            echo "location.href='../vistas/?id=new_fac_aprobadas_aprobadas&cot=".$max."&cli=".$_POST['id_cli']."'";
                                            echo "</script>";
                                     ...
                                     if(isset($_GET['editar'])){
                                        ...
                                        mysql_query($sqlr, $conexion);
                                                echo '<script lanquage="javascript">alert("Se ha editado exitosamente");location.href="../vistas/?id=new_fac_aprobadas_aprobadas&cot='.$_GET['cot'].'&cli='.$_POST['id_cli'].' "</script>'; 
                                     }
                           if(isset($_GET['cot'])){         ?>
...
?> 
     ...
     <?php if(isset($_GET['cot'])){ 
          include '../vistas/aprobadas/generar_orden_cot_2.php'; 
}
                        ...
                        <div class="span12 widget dark stacked">
                            <header>
                                <h4 class="title">Ordenes de Produccion</h4>
                                        ...
                                        <?php
$request_ac=mysql_query('select * from orden_produccion where referencia='.$orden_p.' order by id_orden desc ');
										...
										<div class="span12 widget lime">
											<section class="body">
												<div class="body-inner no-padding">
													<?php
														$request = mysql_query("SELECT * FROM facturas WHERE numero_pedido = '$orden_p' ORDER BY id_factura desc");     
														if ($request) {
															//echo'<hr>';
															$table = '<table class="table table-bordered table-striped table-hover" id="">';
															$table = $table . '<thead>';
															...
															while ($row = mysql_fetch_array($request)) {
																...
																if ($ver_ped == 'Habilitado') {
																	$verp = '<a href="../vistas/?id=new_fac_aprobadas_aprobadas&cot=' . $row["cotizacion"] . '&cli=' . $row["id_cliente"] . '">';
																} else {
																	$verp = '';
																}

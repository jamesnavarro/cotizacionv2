<?php
$reques27=mysql_query("SELECT * FROM producto a, cotizaciones c, producto_rep_acc b, referencias d where b.id_ref_acc=d.id_referencia and a.id_p=b.id_p and c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]." and d.id_referencia=".$row['id_referencia']." ");


        $cantidad_acc=0;
        $tac2 = 0;
        $peso_acc2 = 0; $cangen = 0;$ff= 0;$aa=0;$mm=0;$porcacc=0;
	while($rowacc=mysql_fetch_array($reques27))
	{            
 ...
 $altura_ventana = $alto - $altura;
	...
	//Por cada resultado pintamos una linea
            ...
            //--------------------------------------------------------------------
                        if($rowacc['can_rej']!=0){
    $request_v2=mysql_query("SELECT * FROM producto a, producto_rep_rej b, referencias c, cotizaciones d where d.id_cot=".$_GET['cot']." and d.id_referencia=a.id_p and b.id_referencia=c.id_referencia and a.id_p=b.id_p and d.id_cotizacion=".$rowacc["id_referencia"]." and b.id_r_rej=".$rowacc['can_rej']." ");
while($rowaccz=mysql_fetch_array($request_v2))
{
    echo '---------------------------<br>';

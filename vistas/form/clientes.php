...
if(isset($_GET['cod'])){ $sql='select * from clientes where id_cli="'.$_GET['cod'].'"'; $fil = mysqli_fetch_array(mysqli_query($conexion,$sql)); $id_p= $fil["id_cli"];
...
<div class="row-fluid">
      ...
      <div class="span4">
       <select name="departamento" id="ciudad"  class="span12">
                 <?php if(isset($_GET['cod'])){echo "<option value='".$Departamento."'>".$Departamento."</option>";}else{echo "<option value=''>..Seleccione el producto</option>"; } ?> 
                  <?php  $consulta= "SELECT * FROM `departamentos` group by nombre_dep"; $result=  mysqli_query($conexion,$consulta); while($fila=  mysqli_fetch_array($result))
                         {
                     $valor1=$fila['cod_dep'];
 ...
 <?php if(isset($_GET['cod'])){$request=mysqli_query($conexion,'select count(*) from cotizacion where id_cliente='.$_GET['cod']);if($request){ $request = mysqli_fetch_row($request); $num_items = $request[0];}else{ $num_items = 0;}$rows_by_page = 10;$last_page = ceil($num_items/$rows_by_page);if(isset($_GET['page'])){ $page = $_GET['page'];}else{	$page = 1;}?> 
            <div class="row-fluid">     
                <!-- START Form Wizard -->    

...
<?php
 ...
 $resu = mysqli_query($conexion,"select numero_cotizacion,version,obra from cotizacion where id_cot = '".$_GET['cot']."' ");
  
 $c = mysqli_fetch_array($resu);
 $num = $c['numero_cotizacion'].'.'.$c['version'];
 $obra = $c['obra']; 
  
 $reques=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c, desgloses d, referencias e where c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]." and d.id_referencia=e.id_referencia and a.id_p=d.id_producto and c.estado_item='Aprobado' ORDER BY e.referencia, c.cantidad_c ASC");
 //$reques=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c where c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]." and estado_item='Aprobado' ORDER BY c.fila ASC ");

...
switch ($_GET['sw']){
    case 1:
              $request2=mysql_query('select * from cont_terceros WHERE cod_ter="'.$_GET["doc"].'"');
        ...
        break;
    case 2:
        $request2=mysql_query('SELECT * FROM producto WHERE codigo="'.$_GET['cod'].'"');
        $row2=mysql_fetch_array($request2);
        $p = array();
        $p[0] = $row2["id_p"];
          $p[1] = $row2["producto"];
        ...
        $ajuste = mysql_query("select * from ajustes where id_referencia=$ref and id_vidrio=$vid ");
        $aj = mysql_fetch_array($ajuste);
        if($aj){
        ...
        $fi31 =mysql_fetch_array(mysql_query($s3));
        ...
        $porcaccB= $fipaB["p"]/100; 
          $request_acE=mysql_query("SELECT * FROM producto a, producto_rep_acc b, referencias c where b.id_ref_acc=c.id_referencia and a.id_p=b.id_p and a.id_p=".$ref." order by b.para ");
        if($request_acE){
        ...
        //--------------------------------------------------------------------
        if($row['can_rej']!=0){
        $request_v2=mysql_query("SELECT * FROM producto a, producto_rep_rej b, referencias c where b.id_referencia=c.id_referencia and a.id_p=b.id_p and a.id_p=".$ref." and b.id_r_rej=".$row['can_rej']." ");
        ...
        $request_mano=mysql_query("SELECT * FROM producto a, producto_rep_ma b, referencia_ma c where b.id_ref_ma=c.id_ref_ma and a.id_p=b.id_p and a.id_p=".$ref);    
        if($request_mano){
        ...
        $req=mysql_query("SELECT * FROM producto a, producto_rep_mo b, referencia_mo c where b.id_ref_mo=c.id_ref_mo and a.id_p=b.id_p and a.id_p=".$ref);
        if($req){
        $tot=0;$tot_fom = 0;
        ...
        $request_ad=mysql_query("SELECT * FROM producto a, producto_rep_ad b, referencia_admin c where b.id_ref_ad=c.id_ref_ad and a.id_p=b.id_p and a.id_p=".$ref);
        ...
        case 6:
            ...
            $sx = "SELECT max(numero_cotizacion) FROM cotizacion";
                                    $filax =mysql_fetch_array(mysql_query($sx));
                ...
                mysql_query("update cotizacion set fecha_guardado='".date("Y-m-d H:m:s")."', registrado='$ase', express='$ser',fecha_de_entrega='$ent',nota='$obs',obra='$obra',validez='$pag', ubicacion='$dir', estado='Pedido por aprobar' where id_cot='$cot'  ");
                                $p = array();
                                     $p[0] = $cot;
                                    $p[1] = 0;
                                    $p[2] = 1;
        ...
        case 7:
        $cot = $_GET['cot'];
        $result = mysql_query("SELECT *, d.estado FROM producto a, cotizaciones c, cotizacion d where c.linea_cot='Vidrio' and c.id_referencia=a.id_p and c.id_cot=d.id_cot and c.id_cot=".$cot."  order by c.fila asc");
        $c = 0;
        $gt= 0;
        $gtiva= 0;
        $ct= 0;
        ...
        <?php
        mysql_query("update cotizacion set costo_total='$gt' where id_cot='$cot' ");
        break;
        case 8:
        $id=$_GET['id'];
        mysql_query("delete from cotizaciones where id_cotizacion='$id' ");
        ...
        $result = mysql_query("select * from cotizaciones where id_cotizacion='$id' ");
        ...
        break;
        case 11:
        $request2=mysql_query('select * from cotizacion a, cont_terceros b WHERE a.id_tercero=b.id_ter and a.id_cot="'.$_GET["cot"].'"');
        $row2=mysql_fetch_array($request2);
        $p = array();
        $p[0] = $row2["id_ter"];
        $p[1] = $row2["cod_ter"];
        ...
        $fecha= date('Y-m-d');
        $ver = mysql_query("select count(id_ajuste), id_ajuste from ajustes where id_referencia=$ref and id_vidrio=$esp  ");
        $v = mysql_fetch_array($ver);
        if($v[0]==0){
            $ok = mysql_query("insert into ajustes (id_referencia, id_vidrio, valor, precio, ajuste, por) values ($ref,$esp,$pre,$und,$aju, '$user')") or die(mysql_error());
            echo $ok;
        }else{
           $id = $v[1];
           mysql_query("update ajustes set precio = $pre, valor= $und, ajuste= $aju, por= '$user' where id_ajuste=$id ");
            echo "2";
        }
        ...
        break;
        case 13:
            if($_SESSION['k_username']=='admin' || $_SESSION['k_username']=='STEFANNYR' || $_SESSION['k_username']=='TATIANA.JULIAO' || $_SESSION['k_username']=='samir'){
                $dis = '';
            }else{
                $dis = 'disabled';
            }
        ...
        $result = mysql_query("SELECT id_ajuste, producto, color_v, valor, ajuste, precio, fecha, por, c.id_referencia, c.id_vidrio FROM producto a, tipo_vidrio b, ajustes c where a.id_p=c.id_referencia and b.id_vidrio=c.id_vidrio ".$esp."  ".$pro." ");
        ...
        $alu= $_GET["alu"];
        $ace= $_GET["ace"];
        mysql_query("update cont_terceros set pvi='$vid', pal='$alu', pac='$ace', autorizacion='".$_SESSION['k_username']."' where id_ter='$id' ");
        ...
        mysql_query("update tipo_vidrio set estado='$est', modi='".$_SESSION['k_username']."' where id_vidrio='$id' ");
        echo $_SESSION['k_username'];
        ...
        $id= $_GET["id"];
        mysql_query("delete from ajustes where id_ajuste='$id' ");
        }
        ...
        $ref= $_GET["ref"];
        $vid= $_GET["vid"];
        $ver = mysql_query("select count(*) from ajustes where id_referencia=$ref and id_vidrio=$vid  ");
        $v = mysql_fetch_array($ver);
        echo $v[0];
        break;
        case 18:
...
$s3 = "SELECT (".$precio.") as p FROM porcentajes where area_por='Vidrio'";
    ...
    case 19:
        ...
        mysql_query("update ajustes set valor='$cos', precio='$pre', ajuste='$aju', por='$use' where id_ajuste='$id' ");
        echo $use;
        break;
        case 20:
            $cot = $_GET['cot'];
        ...
        case 21:
        $cot = $_GET['cot'];
        $query = mysql_query("select * from cotizaciones_materiales where id_cotizacion = '$cot' ");
        $c = 0;
        $nt = 0;
        $tt = 0;
    ...
    case 22:
        $id = $_GET['id'];
        $query = mysql_query("delete from cotizaciones_materiales where id_cot_mat = '$id' ");
        echo $query;
        break;
    case 23:
         $id = $_GET['id'];
          $uti = $_GET['uti'];
           $costo = $_GET['cos'];
           $ver = mysql_query("update referencias set costo_mt='$costo', utilidad='$uti', modificado='".$_SESSION['k_username']."' where id_referencia='$id' ") or die(mysql_error());
           echo $ver;
           break;

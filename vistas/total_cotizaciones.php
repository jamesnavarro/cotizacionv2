<?php
   $request=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c, cotizacion b where b.id_cot=c.id_cot and c.id_referencia=a.id_p and b.registrado like '%".$ciu['usuario']."%' ");
...
if($request){
	...
	{    
            ...
            $compu =mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones_sub c where  c.id_referencia_sub=a.id_p and c.id_cot_sub=".$row["id_cot"]." and c.id_producto_cot=".$row["id_cotizacion"]."");
    $costo_sp = 0; $costo_fom_sp=0; 
    $costo_mp = 0;
          $costo_fom_mp = 0;
    while ($sub=mysqli_fetch_array($compu)){

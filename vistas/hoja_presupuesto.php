...
//    echo'<hr>';
       $table = '<table class="table table-bordered table-striped table-hover" id="">';
             $table = $table.'<thead >';
              ...
              $table = $table.'</tr>';
              $table = $table.'</thead>';
 $requesr=mysql_query("SELECT * FROM producto a, cotizaciones c, producto_rep_rej b, referencias d where b.id_referencia=d.id_referencia and a.id_p=b.id_p and c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]." ");
 $xx=0; $xxfom=0; 
        $peso_rej=0;
	while($row=mysql_fetch_array($requesr))
	{            
            $cons_vi = "SELECT * FROM tipo_vidrio where id_vidrio=".$row['id_vidrio']." ";
                $fv =mysql_fetch_array(mysql_query($cons_vi));
                $cons_vi2 = "SELECT * FROM tipo_vidrio where id_vidrio=".$row['id2_vidrio']." ";
...
$request_maq=mysql_query("SELECT *, sum(cantidad_c) as cc FROM producto a, producto_rep_mo b, referencia_mo c, cotizaciones d where d.id_referencia=a.id_p and b.id_ref_mo=c.id_ref_mo and a.id_p=b.id_p and d.id_cot=".$_GET["cot"]." group by c.id_ref_mo");

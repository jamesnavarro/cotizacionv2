<?php
	...
	$request = mysqli_query($con2,'SELECT COUNT(*) FROM cotizacion a, cont_terceros b WHERE  a.id_tercero = b.id_ter '. $pre . $freg . $cot . $nom . $obr . $reg . $ana . $est );
	if ($request) {
			...
			$v = '';
				...
				if ($row['id_cot'] == 'Personal') {
					...
					$es = $est . '' . $row["estado"];
                                        ...
                                        $btnver = '<button type="button" onclick="ver_costosintalacion('.$row['id_cot'].');">Ver. </button>';

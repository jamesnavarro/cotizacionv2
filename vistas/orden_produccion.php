...
$pdf->SetWidths(array(15, 60, 20, 15, 30, 10, 20, 20));
	...
	$pdf->SetFillColor(230,230,230);
        $pdf->SetTextColor(0);
                         ...
                         $aluminio1 = mysqli_query($conexion,$sq21);
                         $canfilas1 = mysqli_num_rows($aluminio1);
                         if($canfilas1!=0){
        $pdf->Row(array('REF', 'DESCRIPCION DEL PERFIL', 'LADO', 'PERFIL', 'UND', 'CANT. T', 'CANT. PERF', 'MEDIDA', 'VLR. COSTO'));
			...
			$pdf->SetFont('Arial','I',0);
    			...
    			$pdf->SetTextColor(0);
                       ...
                       $r = $r +(($tt2/$fila['cantidad_c'])*$c);
                         ...
                         $aluminio = mysqli_query($conexion,$sq2);
                         $canfilas = mysqli_num_rows($aluminio);
                         ...
                         $ttl =0; $total_vid=0;
                                ...
                                for ($j=0; $j<$canfilas; $j++){
                                    ...
                                    $fa = mysqli_fetch_array($aluminio);
                          if($fa['signo']=='+'){
                if($fa['medida_r_a']==1){
           ...
           $ttl = $ttl + ($al*$mp*(($d))/$n);
                ...
                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< VIDRIO >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                        ...
                        $costo_vidrio= $fi3["costo_v"]/$porcv;
                         ...
                         $vidrioan = mysqli_query($conexion,$sq2);
                         $canfilasv = mysqli_num_rows($vidrioan);
                         $ancho = $m1;
                         $alto = $m2;
                         $ttl =0;
                         ...
                         $total_vid2=0;
                                for ($j=0; $j<$canfilasv; $j++){
                                    $row = mysqli_fetch_array($vidrioan);
         ...
         $vidrio1 = mysqli_query($conexion,$sq2);
         $fil_an = mysqli_fetch_array($vidrio1);
         $id_p= $fil_an["id_p"];
            if($fil_an['signo']=='+'){
           ...
           $total_vid2 = $total_vid2 + $m2*$costo_vidrio*$c;
                ...
                $per_por = "SELECT (".$fila["porcentaje"].") as p FROM porcentajes where area_por='MP' and grupo='Perfiles' ";
                        $fiper =mysqli_fetch_array(mysqli_query($conexion,$per_por));
                        $porcper= $fiper["p"]/100;
                        ...
                        $rejillas = $con->conectar();
                         ...
                         $rejillas = mysqli_query($conexion,$sq2r);
                         $canrej = mysqli_num_rows($rejillas);
                            ...
                            $xx = 0;
                              for ($j=0; $j<$canrej; $j++){
                                   $row = mysqli_fetch_array($rejillas);
       ...
       $request_vrej=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_vid b, referencias c where b.id_ref_vid=c.id_referencia and a.id_p=b.id_p and a.id_p=".$fila['id_p']." and b.id_r_v=".$row["medida_rej"]." ");
            ...
            $numero = ($medrej*$tv2)/$row["medida"];
             $xx = $xx + (($medrej*$tv2*$row["costo_mt"])*$c/1000);
              $pdf->Row(array($fa['referencia'], $fa['descripcion'],  $fa['medida'], number_format($medrej) , number_format($tv2), number_format($numero,1,',','.'), number_format($medrej*$tv2*$row["costo_mt"]*$c/1000)));
                ...
                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ACCESORIOS>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                   $vidrio_por = "SELECT (".$fila["porcentaje"].") as p FROM porcentajes where area_por='Vidrio'";
                        ...
                        $costo_vidrio= $fi3["costo_v"]/$porcv;
                         ...
                         $accesorios = mysqli_query($conexion,$sq2);
                         $canacc = mysqli_num_rows($accesorios);
                ...
                $tac = 0;
               for ($j=0; $j<$canacc; $j++){
                                    $row = mysqli_fetch_array($accesorios);
         //--------------------------------------------------------------------
                        if($row['can_rej']!=0){
    $request_v2=mysqli_query($conexion,"SELECT * FROM producto a, producto_rep_rej b, referencias c where b.id_referencia=c.id_referencia and a.id_p=b.id_p and a.id_p=".$fila['id_p']." and b.id_r_rej=".$row['can_rej']." ");

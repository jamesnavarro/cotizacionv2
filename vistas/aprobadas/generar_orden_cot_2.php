...
//include '../vistas/imprimir_reportes.php';
	...
	$query_total_item = mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c where c.id_referencia=a.id_p and c.id_cot=".$_GET["cot"]."  order by c.fila asc");
	$row_total_item = mysqli_num_rows($query_total_item);
 ?> 
...
if(isset($_GET['cot'])){ 
  if(isset($_POST['buscar'])){   
      $request=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones c WHERE c.id_referencia = a.id_p AND  c.id_cot = " . $_GET["cot"] . " AND CONCAT(producto, referencia_p,tip) LIKE '%" . $_POST['buscar'] . "%' ORDER BY c.fila ASC ");
  }else{
...
if($fv24['color_v']==''){$v24 = '';}else{$v24 = '+'.$fv24['color_v'];}
      ...
      if($row['pelicula']=='No Aplica'){
            ...
            //consulta de compuestos  a005c9f95212f79b7e20753ceeecd8f1     61843d7a6710ba2f2dd19abb2cb04e90
                $com=mysqli_query($conexion,"SELECT * FROM producto a, cotizaciones_sub c where c.id_referencia_sub=a.id_p and c.id_cot_sub=".$_GET["cot"]." and c.id_producto_cot=".$row["id_cotizacion"]."");
                $compuesto ='<ul>';
                $up = '';
				...
				<div class="body-inner">                    
					<?php
						if (isset($_GET['up_serv'])) {
							...
							$request_ac1 = mysqli_query($conexion,"SELECT *
							...
							<tr>
								...
								<td colspan="7">
									...
									<select class="span2" name="color_aluminio" id="color_aluminio">
										...
										<?php
											$query_select_color_aluminio = mysqli_query($conexion,"SELECT * FROM tipo_aluminio");
											while ($row_color_aluminio = mysqli_fetch_assoc($query_select_color_aluminio)) {
							...
							<tbody id="info_vidrios">
								<?php
									if (isset($_GET['up_serv'])) {
										...
										$query_select_info_vidrios = mysqli_query($conexion,"SELECT *
										...
										while ($row_info_vidrios = mysqli_fetch_assoc($query_select_info_vidrios)) {
											...
											$query_select_tipo_vidrio = mysqli_query($conexion,"SELECT * FROM producto WHERE id_p = " . $row_info_vidrios['id_prod']);
											$row_tipo_vidrio = mysqli_fetch_array($query_select_tipo_vidrio);
											echo "<tr>";
							...
							<tbody id="info_accesorios">
								<?php
									if (isset($_GET['up_serv'])) {
										...
										$query_select_info_accesorios = mysqli_query($conexion,"SELECT * FROM info_servicios WHERE id_cot = " . $_GET['cot'] . " AND linea = 'Accesorio' AND id_servicio = " . $_GET['up_serv']);
										while ($row_info_accesorios = mysqli_fetch_assoc($query_select_info_accesorios)) {
											echo "<tr>";
											$query_select_accesorio = mysqli_query($conexion,"SELECT * FROM referencias WHERE id_referencia = " . $row_info_accesorios['id_prod']);
											$row_accesorio = mysqli_fetch_array($query_select_accesorio);

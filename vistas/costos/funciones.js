...
function recalcular(id){
     ...
     $("#operaciontre"+id).val(number_format(valor));
     var precio = $("#precio_base_1").val();
     var precio2 = $("#precio_base_2").val();
     var total = ((valor) / precio) * 100;
     $("#porcen"+id).val(total);
     
//     var po = $("#total_costo_fijos_1").val();
//     var t = parseFloat(po) + parseFloat(total);
//     $("#total_costo_fijos_1").val(t);
//     $("#total_costo_fijos_2").val(t);
     calcular();
}
function calcular(){
            ...
            var ace = $("#mp_acero").val();
            var p_alu = $("#por_alum").val();
            var p_ace = $("#por_ace").val();
            
            var porcentaje_transporte = $("#porcentaje_transporte").val();
            
            var a1 = parseInt((alu / ((100-parseInt(p_alu))/100))-parseInt(alu));
            
            var ac1 = parseInt((ace / ((100-parseInt(p_ace))/100))-parseInt(ace));
            
            $("#desp_alum").val(a1.toFixed(2));
            $("#desp_alum1").val(number_format(a1));
            $("#desp_alum2").val(a1.toFixed(2));
            
            $("#desp_acero").val(ac1.toFixed(2));
            $("#desp_acero1").val(number_format(ac1));
            $("#desp_acero2").val(ac1.toFixed(2));
            ...
            $("#desp_vidrio1").val(number_format(a2));
            ...
            console.log('total accesorios:'+t_acc);
            $("#desp_accesorios").val(a3.toFixed(2));
            $("#desp_accesorios1").val(number_format(a3));
            $("#desp_accesorios2").val(a3.toFixed(2));

            var total_costo = parseFloat(a1) + parseFloat(ac1) + parseFloat(a2) + parseFloat(a3) + parseFloat(alu) + parseFloat(ace) +   parseFloat(vid) + parseFloat(t_acc) + parseFloat(fab) + parseInt(ins) + parseInt(poli) + parseInt(anda);
            $("#total_costo_1").val(total_costo.toFixed(2));
            $("#total_costo_11").val(number_format(total_costo));
            ...
            $("#precio_base_3").val(number_format(precio_base_2));
...
//                              $("#sub_total3"+(parseInt(id) + 1)).val(tp);
                              ...
                              //console.log('base'+base);
                                ...
                                if(util=='Si'){
                                    ...
                                    var rt = parseFloat(t) + parseFloat(t_base);
                                    console.log('item = '+rt);
                                }
                                
                                
                                $("#operacionuno"+id).val(t.toFixed(2));
                                $("#operacionunox"+id).val(number_format(t));
                                $("#operaciondos"+id).val(t.toFixed(2));
                                $("#operaciontre"+id).val(number_format(t));
                                ...
                                if(por_to!=='undefined'){
                                      ...
                                      $("#sub_total3"+id).val(tp2.toFixed(2));
                                       $("#sub_total4"+id).val(number_format(tp2));
                                       $("#sub_total23"+id).val(number_format(tp));
            ...
            $("#precio_venta_base_3").val(number_format(pvbi_2));
                        
                           var utilidad_neta = parseFloat(precio_base_2) - parseFloat(total_costo) - parseFloat(total_utilidad);
                           //("#tgt").html('t.ut:'+total_utilidad);
                           console.log('precio_base_2:'+precio_base_2);
                           console.log('total_costo:'+total_costo);
                           console.log('t.ut:'+total_utilidad);
                           console.log('utilidad_neta:'+utilidad_neta);
                           //alert(total_utilidad);
               console.log(parseFloat(suma_costo));
                          $("#utilidad_1").val(utilidad_neta.toFixed(2));
                          $("#utilidad_2").val(utilidad_neta.toFixed(2));
                          $("#utilidad_3").val(number_format(utilidad_neta));
                          ...
                          console.log("gan_2 = "+gan_2+ " precio_venta_base_2 x "+precio_venta_base_2+" (ganancia_2 /100) "+(ganancia_2 /100));
                          $("#Ganancia_real").val(utilidad_neta.toFixed(2));
                          $("#Ganancia_real2").val(number_format(utilidad_neta));
                          var porcentaje_ganancia = (utilidad_neta / precio_base_2)*100;
                          $("#por_ganancia_real").val(porcentaje_ganancia.toFixed(2));

                          console.log("comision= "+precio_base_2 +"*"+ total_p_comision);
                          var com = precio_base_2 * total_p_comision;
                          $("#comision").val(com.toFixed(2));
                          $("#comision2").val(number_format(com));
                     ...
                     $.ajax({
                            ...
                            success : function(incremento){
                               ...
                               console.log("precio_venta_base_2="+precio_venta_base_2+" * incremento "+(incremento/100)+" + "+parseFloat(gan_2));
                               var ajuste_precio = t_ganancia - utilidad_neta;
                               console.log("ajuste_precio="+(t_ganancia)+' -' + (utilidad_neta));
                              $("#ajuste").val(ajuste_precio.toFixed(2));
                              $("#ajuste2").val(number_format(ajuste_precio));
                              ...
                              $("#por_ajuste").val(por_ajuste.toFixed(2));
                               ...
                               var precio_venta_total = parseFloat(precio_venta_base_2) + parseFloat(com) + parseFloat(ajuste_precio);
                                ...
                                $("#precio_venta").val(precio_venta_total.toFixed(2));
                                 ...
                                 $("#precio_venta_f2").val(number_format(precio_venta_total));
...
function pasar(caja1,caja2){
    var num = $("#"+caja2).val();
    $("#"+caja2).val(number_format(num));
    var pago = parseFloat(num.replace(/[^0-9\.]/g, ''));
//    var cadena = num,
//    patron = ",",
//    nuevoValor    = "",
...
function savecalcular(id_cos,n){
    ...
    $.ajax({
        type:'GET',
        data:'id_cos='+id_cos+'&item='+item+'&cot='+cot+'&por='+por+'&sw=6',
        url:'acciones.php',
        success : function(res){
            alert('Se guardo la modificacion con exito. '+por+' - '+n+ '-'+id_cos+ '| log:'+res);
            console.log(res);
        }
    });
}
function savecalcularUtilidad(id_cos,n){
    var item = $("#item").val();
    var cot = $("#cot").val();
    var por = $("#ganancia_2").val();
    $.ajax({
        type:'GET',
        data:'id_cos='+id_cos+'&item='+item+'&cot='+cot+'&por='+por+'&sw=6',
        url:'acciones.php',
...
function savecalculardesperdicio(id_cos,nom){
    var item = $("#item").val();
    var cot = $("#cot").val();
    var por = $("#"+nom).val();
    $.ajax({

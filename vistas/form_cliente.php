...
$request=mysqli_query($conexion,'select count(*) from clientes');
if($request){
    ...
    <body  onload="javascript:pasar();">
        ...
        <article class="module width_full">
			<header><h3>Busqueda de Clientes</h3></header>
                     ...
                     $request=mysqli_query($conexion,"select * from clientes WHERE id_cli=".$_GET['cod']);
...
if($nom !='' || $tel !='' || $dir !='' || $use !='' || $pro !='' || $dep !='' || $mun !='' || $tip !=''){
$request=mysqli_query($conexion,"select * from sis_empresa WHERE cliente='Si' and nombre_emp LIKE '%".$nom."%' and tel_oficina_emp LIKE '%".$tel."%' and direccione_emp LIKE '%".$dir."%' and departameto_emp LIKE '%".$dep."%' and municipio_emp LIKE '%".$mun."%' and tipo_emp LIKE '%".$tip."%' and rips LIKE '%".$use."%' order by id_empresa asc ".$limit);}
}
else{
$request=mysqli_query($conexion,'select * from clientes '.$limit);
}

<?php 
$request=mysql_query('select count(*) from sis_empresa');
if($request){
...
if(isset($_GET['pasar'])){
    $re=mysql_query("SELECT * FROM sis_empresa ");
    $v = 0;
...
if(isset($_POST['empresa']) || isset($_POST['ciudad']) || isset($_POST['user'])){
    ...
    $request=mysql_query("SELECT * FROM sis_empresa where nombre_emp like '%".$_POST['empresa']."%' and municipio_emp like '%".$_POST['ciudad']."%' and usuario like '%".$_POST['user']."%'");
}else{
$request=mysql_query("SELECT * FROM sis_empresa ");
  }

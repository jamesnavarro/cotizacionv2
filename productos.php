        ...
        <ul>
                ...
                <?php
                  ...
                  while ($row = mysqli_fetch_array($result)) {
                      ...
                      $busqueda = mysqli_query($conexion,"select count(codigo) from producto where codigo='$cod' ");
                      ...
                      if($b[0]==0){
                          ...
                          $id = mysqli_insert_id($conexion); // maximo id 
                     ...
                     if($b[0]==0){
                      ...
                      echo '<li>-----------------------------perfiles-----------------------------------------</li>';
                       while($f = mysqli_fetch_array($resultper)){
                           ...
                           $result_perfil = mysqli_query($conexion,"SELECT id_referencia FROM `referencias` where referencia='".$f['referencia']."' group by referencia");
                           $rp = mysqli_fetch_array($result_perfil);
                           $ref = $rp[0];
                           ...
                           $error = mysqli_query($conexion,$sql2) ;
                        ...
                        while($r = mysqli_fetch_array($resultf)){
                            echo '<li>'.$r[2].'-'.$r[3].'</li>';
                            $result_acc = mysqli_query($conexion,"SELECT id_referencia FROM `referencias` where codigo='".$r['codigo_pro']."' group by referencia");
                           $ra = mysqli_fetch_array($result_acc);
                           $ref = $ra[0];

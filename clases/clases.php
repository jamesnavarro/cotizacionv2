<?php 
 ...
 $request=mysql_query("SELECT *, (a.producto) as producto FROM producto a, cotizaciones c, orden_detalle e, procesos_activos f where  e.anula='0' and f.id_orden_d=e.id_orden_d and e.id_producto=a.id_p and c.id_referencia=a.id_p  and f.id_op=".$op." and e.parte_otro=0 group by f.barra ");
...
//          echo number_format($mult2).'<br>';
            ...
            if($row["id_prod_cambio"]!=0){
                $name = mysql_query("select * from producto where id_p=".$row["id_prod_cambio"]." ");
...
<input type="hidden" value="<?php echo $cont ?>" id="contd"> 
        <?php
         $request=mysql_query("SELECT *, (a.producto) as producto FROM producto a, cotizaciones c, orden_detalle e, procesos_activos f where e.parte_otro=1 and e.anula='0' and f.id_orden_d=e.id_orden_d  and c.traz_vid=a.id_p  and c.id_cot=".$_GET["cot"]." and e.codigo=".$_GET['op']."  group by f.id_orden_d");
	...
	while($row=mysql_fetch_array($request))
	{
            ...
            if($row["linea_cot"]=='Aluminio'){
               ...
               $s3p = "SELECT * FROM producto where id_p=".$row["id_producto"]." ";
                $fi3p =mysql_fetch_array(mysql_query($s3p));
    ...
    function listaproductos($op,$cot,$cli){
        ...
        $r = mysql_fetch_array($request);
            if(isset($_POST['producto'])){
                $request=mysql_query("SELECT * FROM producto a, cotizaciones c where  c.id_referencia=a.id_p  and c.id_cot=".$_GET["cot"]." and c.id_cotizacion=".$_POST['producto']." ");
            }else{
                $request=mysql_query("SELECT * FROM producto a, cotizaciones c where  c.id_referencia=a.id_p  and c.id_cot=".$_GET["cot"]);  
            }   
if($request){ ?>

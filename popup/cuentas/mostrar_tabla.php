...
$request=mysqli_query($conexion,'SELECT count(*) FROM referencias where grupo="Perfileria" '.$col.' ');
            if($request){
                ...
                ?>
                            <table class="table table-bordered table-condensed table-hover">
                                ...
                                <tbody>
                                    ...
                                    $sql = mysqli_query($conexion,"SELECT * FROM `referencias` where grupo='Perfileria'  $col ".$limit);
			$item = 0;
...
<td><a href="../popup/cuentas.php?codigo='.$mostrar['id_referencia'].'">'.$mostrar['referencia'].'</a></td>
<td>'.$mostrar['descripcion'].'</td><td>'.$mostrar['dado'].'</td>'; }
			}else{

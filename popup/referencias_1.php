...
<script src="../popup/referencias_1/funciones.js"></script>
<script language="javascript" type="text/javascript">

function pasar(cod,ref,des){
    var id = $("#in").val();
   window.opener.$("#xcod"+id).val(cod);
   window.opener.$("#xref"+id).val(ref);
   window.opener.$("#xdes"+id).val(des);
   
   window.opener.$("#upcod"+id).val(cod);
   window.opener.$("#upref"+id).val(ref);
   window.opener.updateref(id);
   window.close();
}
</script>
...
<a href="" title="pasar valor" onload="javascript:pasar();"><input type="button" name="cerrar" value="Cargar"  onclick="window.close();"></a>  
     ...
     <?php }
         ...
         include '../modelo/conexion.php';
         ?>
Buscar:<input type="text" id="buscar_empleado" autofocus placeholder="Codigo o Descripcion">
<input type="text" id="in" value="<?php echo $_GET['id'] ?>" disabled><br>

<div class="datagrid" id="empleados">
    <?php        include '../popup/referencias_1/mostrar_tabla.php';  ?>
